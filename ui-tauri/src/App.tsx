...
const WorkspacePanel = lazy(() =>
  import("./panels/WorkspacePanel").then((m) => ({
    default: m.WorkspacePanel,
  })),
);
const OverlayPanel = lazy(() =>
  import("./panels/OverlayPanel").then((m) => ({ default: m.OverlayPanel })),
);
const EnginesPanel = lazy(() =>
  import("./panels/EnginesPanel").then((m) => ({ default: m.EnginesPanel })),
);
...
import {
  FORCE_ONBOARDING_TEST,
  FORCE_PROFILE_INTRO_LOOP,
  FORCE_PROFILE_INTRO_TEST,
} from "./config/debugFlags";
...
function AppShell() {
  ...
  const [toolbarMetaOpen, setToolbarMetaOpen] = useState<
    Record<string, boolean>
  >({});
  const [conceptATab, setConceptATab] = useState<"workspace" | "calibration">(
    "workspace",
  );
  ...
  const compactMeta = useMemo(() => {
    ...
    const src =
      languageLabels[settings?.src_language ?? "auto"] ??
      String(settings?.src_language ?? "AUTO").toUpperCase();
    const tgt =
      languageLabels[settings?.tgt_language ?? "tr"] ??
      String(settings?.tgt_language ?? "TR").toUpperCase();
    ...
    runtimeEngine,
    settings?.ocr_engine,
    settings?.performance_tier,
    settings?.src_language,
    settings?.tgt_language,
  ...
  const startupIntroMode = hasCustomizedSettings
    ? "profileMode"
    : "defaultMode";
  if (!settings && !(FORCE_ONBOARDING_TEST && !forceOnboardingBypassed))
    return (
      <StartupIntro
        mode="defaultMode"
        hold
  ...
  if (FORCE_PROFILE_INTRO_TEST) {
    return (
      <StartupIntro
        key={profileIntroLoopKey}
        mode="profileMode"
        profileName={userProfile?.display_name ?? null}
        avatarDataUrl={userProfile?.avatar_data_url ?? null}
        profileLines={startupProfileLines}
        onComplete={() => {
          if (FORCE_PROFILE_INTRO_LOOP)
            setProfileIntroLoopKey((current) => current + 1);
        }}
      />
    );
  }
  if (
    ...
    return (
      <Onboarding
        onComplete={() => {
          setForceOnboardingBypassed(true);
          setActivePage("canvasA");
        }}
      />
    );
  }
          ...
          <div className="glass-panel relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-none">
            ...
            >
              <div className="relative z-[1] flex min-w-0 flex-1 items-center gap-3">
                ...
                >
                  {compactMeta.map((item, index) => (
                    <div
                      ...
                      onMouseEnter={() =>
                        setToolbarMetaOpen((current) => ({
                          ...current,
                          [item.key]: true,
                        }))
                      }
                      style={{
                        width: toolbarMetaOpen[item.key]
                          ? `${TOOLBAR_META_EXPANDED_WIDTH}px`
                          : "30px",
                        height: "30px",
                        borderRadius: "999px",
                        color: toolbarMetaOpen[item.key]
                          ? "#ffffff"
                          : "rgba(255, 255, 255, 0.65)",
                        background: toolbarMetaOpen[item.key]
                        ...
                        transform:
                          activeDisplayPanel !== "canvasA"
                            ? "translateY(0) scale(1)"
                            : "translateY(-2px) scale(0.965)",
                        filter:
                          activeDisplayPanel !== "canvasA"
                            ? "blur(0px)"
                            : "blur(2.5px) saturate(0.86)",
                        clipPath:
                          activeDisplayPanel !== "canvasA"
                        ...
                        willChange:
                          "opacity, transform, filter, clip-path, width",
                      }}
                    >
                      <div
                        style={{
                      ...
                      <span
                        style={{
                          ...
                          transform: toolbarMetaOpen[item.key]
                ...
                <div
                  className="pointer-events-none absolute left-1/2 z-[2] flex min-w-0 -translate-x-1/2 items-center justify-center"
                ...
                >
                  <div
                    style={{
                      ...
                      left: "50%",
                      top: "50%",
                      transform: !showConceptTabs
                        ? "translateX(-50%) translateY(-50%) scale(1)"
                        : "translateX(-50%) translateY(-52%) scale(0.965)",
                      opacity: !showConceptTabs ? 1 : 0,
                      filter: !showConceptTabs
                        ? "blur(0px)"
                        : "blur(2.5px) saturate(0.86)",
                  ...
                  <div
                    style={{
                      ...
                      borderRadius: "999px",
                      background:
                        "linear-gradient(180deg, rgba(4, 8, 14, 0.82), rgba(8, 12, 18, 0.76))",
                      boxShadow:
                        "inset 0 2px 8px rgba(0,0,0,0.34), inset 0 -1px 0 rgba(255,255,255,0.03)",
                      padding: "4px",
                      opacity: showConceptTabs ? 1 : 0,
                      transform: showConceptTabs
                        ? "translateY(0) scale(1)"
                  ...
                  >
                    <div
                      style={{
                        ...
                        bottom: "4px",
                        left:
                          conceptATab === "calibration"
                            ? "calc(50% + 2px)"
                            : "4px",
                        width: "calc(50% - 6px)",
                        borderRadius: "999px",
                        background:
                          "linear-gradient(180deg, rgba(119, 205, 255, 0.14), rgba(119, 205, 255, 0.09))",
                        boxShadow:
                          "inset 0 1px 0 rgba(255,255,255,0.05), 0 0 0 1px rgba(119, 205, 255, 0.06)",
            ...
            {notices.length > 0 && (
              <Toaster notices={notices} dismissNotice={dismissNotice} />
            )}
            ...
            >
              {(Object.keys(renderPanel) as PageType[]).map((page) => {
                ...
                return (
                  ...
                  >
                    <Suspense fallback={null}>{renderPanel[page]}</Suspense>
                  </div>
                );
              })}
            </div>
