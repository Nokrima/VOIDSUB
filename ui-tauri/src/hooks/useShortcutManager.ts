...
function emitShortcutFeedback(
  ...
  wsClient.injectEvent("shortcut_feedback", {
    shortcut: action,
    state,
    timestamp,
    message,
...
export function validateShortcutKey(
  ...
  if (BLOCKED_KEYS.has(newKey)) {
    return {
      valid: false,
      error: `${newKey} sistem tuşu olduğu için atanamaz.`,
    };
  }
...
export async function syncHotkeysToRust(
  ...
  try {
    ...
    wsClient.injectEvent("log_entry", {
      ...
      prefix: "SHORTCUT",
      code: "SK-010",
      message: `Tuş atamaları güncellendi: ${Object.entries(shortcuts)
        .map(([k, v]) => `${k}=${v}`)
        .join(", ")}`,
    });
  } catch (err) {
...
export function useShortcutManager(deps: ShortcutManagerDeps) {
  ...
  const handleStartStop = useCallback(() => {
    if (!isTranslatingRef.current && !hasSelectedRegionRef.current) {
      emitShortcutFeedback(
        "start_stop",
        "error",
        "Tarama alanı seçilmeden çeviri başlatılamaz.",
      );
      notify(
        "warning",
        "Tarama alanı seçilmeden çeviri başlatılamaz.",
        "shortcut:start_stop:no_region",
      );
      return;
    }
    emitShortcutFeedback("start_stop", "active");
    wsClient.send(
      isTranslatingRef.current ? "stop_translation" : "start_translation",
    );
  ...
  const handleSelectRegion = useCallback(async () => {
    if (temporaryRegionActiveRef.current) {
      emitShortcutFeedback(
        "select_region",
        "error",
        "Geçici alan aktifken ana tarama alanı değiştirilemez.",
      );
      notify(
        "warning",
        "Geçici alan aktifken ana tarama alanı değiştirilemez.",
        "shortcut:select_region:temp_active",
      );
      return;
    }
    ...
    window.setTimeout(
      () => emitShortcutFeedback("select_region", "success"),
      150,
    );
  ...
  const handleHideOverlay = useCallback(() => {
    if (!isTranslatingRef.current) {
      emitShortcutFeedback(
        "hide_overlay",
        "error",
        "Çeviri Katmanı gizleme için aktif çeviri gerekir.",
      );
      notify(
        "warning",
        "Çeviri Katmanı gizleme için aktif çeviri gerekir.",
        "shortcut:hide_overlay:no_translation",
      );
      return;
    }
    ...
    window.setTimeout(
      () => emitShortcutFeedback("hide_overlay", "success"),
      150,
    );
  }, [notify]);
  ...
  const handleTemporaryRegion = useCallback(async () => {
    if (temporaryRegionActiveRef.current) {
      ...
      window.setTimeout(
        () => emitShortcutFeedback("temporary_region", "success"),
        150,
      );
      return;
    }
    if (!hasSelectedRegionRef.current) {
      emitShortcutFeedback(
        "temporary_region",
        "error",
        "Geçici alan kullanmadan önce ana tarama alanı seçilmelidir.",
      );
      notify(
        "warning",
        "Geçici alan kullanmadan önce ana tarama alanı seçilmelidir.",
        "shortcut:temporary_region:no_region",
      );
      return;
    }
    emitShortcutFeedback("temporary_region", "active");
