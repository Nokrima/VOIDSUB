...
import {
  useAppWebSocket,
  translationReasonMessage,
  type OcrFrameStat,
  type OverlaySettingsState,
  type TranslationPreview,
} from "../bridge/useWebSocket";
...
type AppContextValue = {
  ...
  setActivePage: (
    page: "canvasA" | "canvasB" | "canvasC" | "canvasSettings",
  ) => void;
...
export function AppProvider({ children }: { children: ReactNode }) {
  const [activePage, setActivePage] = useState<
    "canvasA" | "canvasB" | "canvasC" | "canvasSettings"
  >("canvasA");
  ...
  const enqueueNotice = useCallback(
    (tone: NoticeItem["tone"], message: string, dedupeKey?: string) => {
      setNotices((current) => {
        const filtered = dedupeKey
          ? current.filter((item) => item.dedupeKey !== dedupeKey)
          : current;
        const duplicate = filtered.find((item) => item.message === message);
        if (duplicate) return filtered;
        return [
          { id: ++noticeIdRef.current, tone, message, dedupeKey },
          ...filtered,
        ].slice(0, 5);
      });
    },
  ...
  const handleToggleTranslation = () => {
    if (!isTranslating && !hasSelectedRegion) {
      enqueueNotice(
        "warning",
        "Çeviri başlatmadan önce bu oturumda tarama alanı seçilmelidir.",
      );
      return;
    }
  ...
  useAppWebSocket({
    onSettings: (data) => {
      ...
      const nextSettings = {
        ...appData,
        ocr_engine: VALID_OCR_ENGINES.has((appData as any).ocr_engine)
          ? (appData as any).ocr_engine
          : "easy",
      ...
      restoreWindowSettingRef.current =
        nextSettings.restore_window_after_region_selection ?? true;
    ...
    onHardware: (data) =>
      setHardware({
        ...data,
        available_engines: (data.available_engines ?? []).filter(
          (engine: string) => VALID_OCR_ENGINES.has(engine),
        ),
      }),
    onOverlaySettings: (data) =>
      setOverlaySettings(data as OverlaySettingsState),
    onOfflineStatus: (data) =>
      setOfflineStatus({
        available: Boolean(data.available),
        busy: Boolean(data.busy),
        selected_model:
          String(data.selected_model ?? "opus_mt_en_tr")
            .trim()
            .toLowerCase() === "nllb"
            ? "nllb"
            : "opus_mt_en_tr",
        models_ready:
          typeof data.models_ready === "object" && data.models_ready
            ? (data.models_ready as Record<string, boolean>)
            : {},
      }),
    onTranslation: (data) => {
    ...
    onTranslationState: (data) => {
      ...
      if (reasonKey === "engine_unavailable") {
        ...
        setActivePage("canvasC");
        const errDetail =
          (data as { message?: unknown }).message || "Bilinmeyen hata";
        enqueueNotice(
          "error",
          `Kritik Uyarı: Ekran yakalama modülü bozuk! Detay: ${errDetail}`,
          "capture_unavailable_redirect",
        );
      } else {
        ...
        if (reasonMessage)
          enqueueNotice(
            "warning",
            reasonMessage,
            `translation_state:${reasonMessage}`,
          );
      }
    },
    onEngineDenied: (data) =>
      enqueueNotice(
        "warning",
        String(data.reason ?? "Seçilen motor açılamadı."),
      ),
    ...
    onEngineRepair: (data) =>
      enqueueNotice(
        Boolean(data.success) ? "success" : "warning",
        String(data.message ?? "Motor onarım sonucu alınamadı."),
      ),
    onOfflineError: (data) =>
      enqueueNotice(
        "warning",
        String(data.message ?? "Offline model işlemi tamamlanamadı."),
      ),
    onOfflineComplete: () =>
      enqueueNotice("success", "Offline modeller kullanıma hazır."),
    onTranslationFallback: (data) =>
      enqueueNotice(
        "warning",
        `${String(data.from ?? "bilinmeyen")} yerine ${String(data.to ?? "yedek motor")} kullanılıyor.`,
      ),
    onOcrRuntimeFallback: (data) => {
      ...
      enqueueNotice(
        "info",
        `${selected} ${sceneLabel} için uygun değil. ${runtime} kullanılıyor.`,
        `ocr_runtime_fallback:${selected}:${runtime}:${sceneMode}`,
      );
    },
    ...
    onRegionSelected: (data) => {
      ...
      const region =
        data && typeof data === "object" && "region" in data
      ...
      if (region && typeof region === "object") {
        setSettings((current) =>
          current
            ? {
                ...current,
                last_region: region as AppSettingsState["last_region"],
                target_region: region as AppSettingsState["target_region"],
              }
            : current,
        );
      }
    ...
    onRegionFailed: (data) => {
      ...
      enqueueNotice(
        "warning",
        String(data.message ?? "Tarama alanı seçimi başlatılamadı."),
      );
    },
    onAsyncError: (data) => {
  ...
  useEffect(() => {
    ...
    listen("tray-exit-requested", async () => {
      ...
      unlistenTrayExitRequested = unlisten;
    });

    const handleStorage = (event: StorageEvent) => {
      if (event.key === "has-selected-region")
        setHasSelectedRegion(event.newValue === "true");
    };
  ...
  useEffect(() => {
    ...
    const offCalibrationFailed = onEvent(
      "calibration_region_failed",
      (data: { message?: string }) => {
        restoreFromCalibration().catch(() => undefined);
        enqueueNotice(
          "error",
          String(data?.message ?? "Kalibrasyon alanı seçimi başarısız oldu."),
        );
      },
    );
    return () => {
  ...
  useEffect(() => {
    const offTempState = onEvent(
      "temporary_region_state",
      (data: { active?: boolean }) => {
    ...
    const offTempCancelled = onEvent("temporary_region_cancelled", () => {
      ...
      if (
        hasSelectedRegionRef.current &&
        resumeTranslationAfterTempSelectRef.current
      ) {
        wsClient.send("start_translation");
      }
      resumeTranslationAfterTempSelectRef.current = false;
    });
    const offTempFailed = onEvent(
      ...
      (data: { message?: string }) => {
        ...
        enqueueNotice(
          "warning",
          String(data?.message ?? "Geçici çeviri alanı başlatılamadı."),
        );
        if (
          hasSelectedRegionRef.current &&
          resumeTranslationAfterTempSelectRef.current
  ...
  useEffect(() => {
    ...
    const timeoutIds = notices.map((notice) =>
      window.setTimeout(() => dismissNotice(notice.id), 3800),
    );
    return () =>
  ...
  useEffect(() => {
    ...
    const isEngineReady = hardware.available_engines?.includes(
      settings.ocr_engine,
    );
    if (!isEngineReady) {
      setActivePage("canvasC");
      enqueueNotice(
        "error",
        "Kritik Uyarı: Seçili OCR motoru çalıştırılamıyor! Lütfen onarın veya indirin.",
        "startup_engine_check",
      );
    }
  ...
  useEffect(() => {
    ...
    return () =>
      document.removeEventListener("keydown", handleEmergencyQuit, true);
  }, []);
  ...
  useEffect(() => {
    hasSelectedRegionRef.current = hasSelectedRegion;
  }, [hasSelectedRegion]);
  ...
  const hasCustomizedSettings = Boolean(
    ...
    (settings.minimize_to_tray !== DEFAULT_APP_SETTINGS.minimize_to_tray ||
      ...
      settings.ocr_filters_enabled !==
        DEFAULT_APP_SETTINGS.ocr_filters_enabled ||
      settings.raw_translation_flow_enabled !==
        DEFAULT_APP_SETTINGS.raw_translation_flow_enabled ||
      settings.active_calibration_profile_id !==
        DEFAULT_APP_SETTINGS.active_calibration_profile_id ||
      JSON.stringify(settings.custom_calibration_profiles ?? []) !==
        JSON.stringify(DEFAULT_APP_SETTINGS.custom_calibration_profiles) ||
      settings.src_language !== DEFAULT_APP_SETTINGS.src_language ||
      settings.tgt_language !== DEFAULT_APP_SETTINGS.tgt_language ||
      settings.shortcuts.start_stop !==
        DEFAULT_APP_SETTINGS.shortcuts.start_stop ||
      settings.shortcuts.select_region !==
        DEFAULT_APP_SETTINGS.shortcuts.select_region ||
      settings.shortcuts.hide_overlay !==
        DEFAULT_APP_SETTINGS.shortcuts.hide_overlay ||
      settings.shortcuts.temporary_region !==
        DEFAULT_APP_SETTINGS.shortcuts.temporary_region ||
      (overlaySettings
        ? overlaySettings.mode !== DEFAULT_OVERLAY_SETTINGS.mode ||
          overlaySettings.font_family !==
            DEFAULT_OVERLAY_SETTINGS.font_family ||
