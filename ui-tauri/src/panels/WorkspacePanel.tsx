...
export const WorkspacePanel: React.FC<WorkspacePanelProps> = ({
  ...
  const [performanceOptions, setPerformanceOptions] = useState<
    ConceptAPerformanceOption[]
  >(conceptABasePerformanceOptions);
  const [currentPerformanceId, setCurrentPerformanceId] =
    useState<string>("Performans");
  const [currentEngineId, setCurrentEngineId] =
    useState<ConceptAEngineId>("easy");
  const lastRequestedTabRef = React.useRef<ConceptATab>(activeTab);

  const engineOptions = React.useMemo<ConceptAEngineOption[]>(() => {
    const available = new Set(
      (hardware?.available_engines ?? []).filter(
        (engine): engine is ConceptAEngineId =>
          engine === "easy" || engine === "winonly",
      ),
    );
    const filtered = workspaceEngineOptions.filter((option) =>
      available.has(option.id),
    );
    if (filtered.length > 0) return filtered;
    return workspaceEngineOptions.filter(
      (option) => option.id === currentEngineId,
    );
  ...
  useEffect(() => {
    const nextBaseId = getConceptABasePerformanceIdFromSettings(
      settings?.performance_tier,
    );
    const customProfileId = settings?.active_calibration_profile_id;
    if (
      customProfileId &&
      performanceOptions.some((item) => item.id === customProfileId)
    ) {
    ...
    <>
      <div
        style={{
          position: "absolute",
          inset: 0,
          visibility: displayedTab === "calibration" ? "visible" : "hidden",
          pointerEvents: displayedTab === "calibration" ? "auto" : "none",
          opacity: displayedTab === "calibration" ? 1 : 0,
        }}
      >
        <CalibrationView
      ...
      <div
        style={{
          position: "absolute",
          inset: 0,
          visibility: displayedTab === "workspace" ? "visible" : "hidden",
          pointerEvents: displayedTab === "workspace" ? "auto" : "none",
          opacity: displayedTab === "workspace" ? 1 : 0,
        }}
      >
        <WorkspaceView
