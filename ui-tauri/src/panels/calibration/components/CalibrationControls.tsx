...
export const CalibrationSliderControl = ({
  ...
  const percent =
    item.max > item.min
      ? clampNumber(((value - item.min) / (item.max - item.min)) * 100, 0, 100)
      : 0;
  const setFromClientX = (clientX: number, element: HTMLDivElement) => {
    ...
    onChange(
      item.key,
      normalizeCalibrationValue(item.min + ratio * (item.max - item.min), item),
    );
  };
  const adjustByStep = (direction: -1 | 1) => {
    onChange(
      item.key,
      normalizeCalibrationValue(value + direction * item.step, item),
    );
  };
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          boxShadow:
            "inset 0 1px 2px rgba(0,0,0,0.34), inset 0 0 0 1px rgba(255,255,255,0.045)",
          overflow: "visible",
      ...
      >
        <div
          style={{
            ...
            background: disabled
              ...
              : `linear-gradient(90deg, ${item.accentStart}, ${item.accentEnd})`,
        ...
        <div
          style={{
            ...
            background: disabled
              ? "linear-gradient(180deg, #b7c1ce, #748194)"
              : "linear-gradient(180deg, #f8fbff, #b9cce3)",
            boxShadow: disabled
              ? "0 4px 10px rgba(0,0,0,0.22), 0 0 0 3px rgba(15,23,42,0.82)"
              : `0 5px 14px rgba(0,0,0,0.32), 0 0 0 3px rgba(15,23,42,0.82), 0 0 18px ${item.glow}`,
...
export const ImprovementToggleBlock = ({
  ...
  const toggleCard = () =>
    setMode((current) => (current === "filters" ? "rawFlow" : "filters"));
  ...
  const title = isFiltersMode
    ...
    : rawFlowEnabled
      ? "Ham Çeviri Akışı"
      : "Filtrelenmiş Çeviri Akışı";
  const subtitle = isFiltersMode
    ? filtersEnabled
      ? "Ön işleme katmanları görüntüyü okumaya hazırlar."
      : "Görüntü doğrudan OCR katmanına gönderilir."
    : rawFlowEnabled
      ? "Metin, ek filtreler olmadan doğrudan çeviriye gider."
      : "Kalite ve tekrar denetimleri çeviri akışını düzenler.";
  const offIcon = isFiltersMode
    ? "M4 7h16M4 12h16M4 17h16"
    : "M4 6h16M6 12h12M8 18h8";
  const onIcon = isFiltersMode
  ...
  return (
    <div
      ...
      style={{
        ...
        transition:
          "box-shadow 180ms ease, opacity 280ms ease, background 280ms ease",
          ...
          <div style={{ display: "grid", gap: 6 }}>
            ...
            <div
              style={{
                height: 1,
                background:
                  "linear-gradient(90deg, rgba(169,189,216,0.28) 0%, rgba(169,189,216,0.12) 40%, rgba(169,189,216,0) 100%)",
              }}
            />
          </div>
