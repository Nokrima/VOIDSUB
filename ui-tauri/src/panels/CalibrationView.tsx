...
import {
  conceptABasePerformanceTiers,
  getSettingsPerformanceKeyFromConceptA,
  type ConceptAPerformanceOption,
  type ConceptAPerformanceTier,
} from "../config/workspacePerformance";
import type {
  AppCustomCalibrationProfile,
  AppPerformanceTier,
} from "../config/uiDefaults";
import {
  type ConceptAEngineOption,
  workspaceEngineLabels,
} from "../config/workspaceEngine";
import {
  applyOcrFilterOverride,
  getCalibrationPreset,
  toRuntimeCalibrationPayload,
  type CalibrationRuntimeValues,
  type PerformanceTier,
} from "../config/calibrationPresets";
...
export const CalibrationView: React.FC<{
  ...
  onPerformanceOptionsChange?: (
    nextOptions: ConceptAPerformanceOption[],
  ) => void;
  ...
  const { notify, settings, isTranslating, handleStartCalibrationSelect } =
    useAppContext();
  ...
  const openZoom = (e: React.MouseEvent, src: string) => {
    ...
    const tw = Math.round(window.innerWidth * 0.9);
    ...
    requestAnimationFrame(() =>
      requestAnimationFrame(() => setZoomAnimated(true)),
    );
  };
  ...
  const currentMotorIndex = engineOptions.findIndex(
    (item) => item.id === currentEngineId,
  );
  const safeMotorIndex = currentMotorIndex >= 0 ? currentMotorIndex : 0;
  const currentMotor =
    engineOptions[safeMotorIndex]?.label ?? workspaceEngineLabels.easy;
  const previousMotor =
    engineOptions.length > 1
      ? (engineOptions[
          (safeMotorIndex - 1 + engineOptions.length) % engineOptions.length
        ]?.label ?? null)
      : null;
  const nextMotor =
    engineOptions.length > 1
      ? (engineOptions[(safeMotorIndex + 1) % engineOptions.length]?.label ??
        null)
      : null;
  const shiftMotor = (dir: -1 | 1) => {
    if (isTranslating)
      return notify(
        "warning",
        "Çeviri aktifken OCR motoru değiştirilemez",
        "settings_lock",
      );
    if (engineOptions.length === 0) return;
    const nextIndex =
      (safeMotorIndex + dir + engineOptions.length) % engineOptions.length;
  ...
  const performanceList = React.useMemo(() => {
    ...
    for (const tier of conceptABasePerformanceTiers) {
      ...
      const customs =
        settings?.custom_calibration_profiles?.filter(
          (p) => p.base_tier === getSettingsPerformanceKeyFromConceptA(tier),
        ) || [];
  ...
  const [performanceArrowHover, setPerformanceArrowHover] =
    useState<HoverState>(null);
  const lastLoadedPerfIdRef = useRef<string | null>(null);
  const currentPerfIndex = performanceList.findIndex(
    (p) => p.id === externalPerformanceId,
  );
  ...
  const previousPerformance =
    performanceList[
      (safePerfIndex - 1 + performanceList.length) % performanceList.length
    ].name;
  const nextPerformance =
    performanceList[(safePerfIndex + 1) % performanceList.length].name;
  ...
  const [rawTranslationFlowEnabled, setRawTranslationFlowEnabled] =
    useState(false);
  const [calibrationDraftProfile, setCalibrationDraftProfile] =
    useState<ConceptCalibrationDraftProfile | null>(null);
  const [calibrationInfoEnabled, setCalibrationInfoEnabled] = useState(false);
  const [calibrationInfoDockMounted, setCalibrationInfoDockMounted] =
    useState(false);
  const [calibrationInfoDockVisible, setCalibrationInfoDockVisible] =
    useState(false);
  ...
  const [activeCalibrationInfoKey, setActiveCalibrationInfoKey] =
    useState<CalibrationInfoKey>("overview");
  const currentRuntimeValues = (
    values = calibrationValues,
    filtersEnabled = ocrFiltersEnabled,
  ) => runtimeValuesFromConcept(values, filtersEnabled);
  const currentBaseRuntimeValues = (
    tier: ConceptAPerformanceTier,
    nextSceneType = sceneType,
    filtersEnabled?: boolean,
  ) => {
    const runtimeTier = getSettingsPerformanceKeyFromConceptA(
      tier,
    ) as PerformanceTier;
  ...
  const schedulePreview = (
    runtimeValues: CalibrationRuntimeValues,
    immediate = false,
  ) => {
  ...
  useEffect(() => {
    setSceneType(
      settings?.ocr_scene_mode === "floating" ? "floating" : "striped",
    );
  ...
  useEffect(() => {
    setRawTranslationFlowEnabled(
      settings?.raw_translation_flow_enabled ?? false,
    );
  ...
  useEffect(() => {
    ...
    const offPreview = onEvent(
      "calibration_preview_result",
      (data: CalibrationPreviewResult) => {
  ...
  const applySceneType = (nextSceneType: "floating" | "striped") => {
    if (isTranslating)
      return notify(
        "warning",
        "Çeviri aktifken sahne tipi değiştirilemez",
        "settings_lock",
      );
    ...
    schedulePreview(
      currentRuntimeValues(calibrationValues, ocrFiltersEnabled),
      true,
    );
  };
  ...
  const shiftPerformance = (dir: -1 | 1) => {
    if (isTranslating)
      return notify(
        "warning",
        "Çeviri aktifken performans ayarı değiştirilemez",
        "settings_lock",
      );
    const nextIdx =
      (safePerfIndex + dir + performanceList.length) % performanceList.length;
    const nextObj = performanceList[nextIdx];
    onPerformanceChange?.(nextObj.id);

    if (nextObj.isBase) {
      const baseRuntimeValues = currentBaseRuntimeValues(
        nextObj.baseTier,
        sceneType,
      );
      wsClient.send("save_settings", {
        active_calibration_profile_id: null,
        performance_tier: getSettingsPerformanceKeyFromConceptA(
          nextObj.baseTier,
        ),
      ...
      const customProfile = settings?.custom_calibration_profiles?.find(
        (p) => p.id === nextObj.id,
      );
      if (customProfile) {
        const filtersEnabled = customProfile.values
          .ocr_filters_enabled as boolean;
        const runtime = runtimeValuesFromConcept(
          customProfile.values as CalibrationValues,
          filtersEnabled,
        );
        wsClient.send("save_settings", {
  ...
  useEffect(() => {
    ...
    const activeOption = performanceList.find(
      (item) => item.id === externalPerformanceId,
    );
    ...
    if (activeOption.isBase) {
      ...
      if (customProfile) {
        ...
        setCalibrationDraftProfile({
          ...
          snapshot: {
            values: customProfile.values as CalibrationValues,
            ocrFiltersEnabled: customProfile.values
              .ocr_filters_enabled as boolean,
          },
          savedSnapshot: {
            values: customProfile.values as CalibrationValues,
  ...
  const updateCalibrationProfileState = (
    nextSnapshot: ConceptCalibrationSnapshot,
  ) => {
    setCalibrationDraftProfile((currentProfile) => {
      const perfObj = performanceList[safePerfIndex];
      const initialSnapshot = {
        values: initialCalibrationValues,
        ocrFiltersEnabled: true,
      };
      if (
        !currentProfile &&
        calibrationSnapshotsMatch(nextSnapshot, initialSnapshot)
      )
        return null;
  ...
  const updateCalibrationValue = (
    ...
    if (isTranslating)
      return notify(
        "warning",
        "Çeviri aktifken kalibrasyon ayarları değiştirilemez",
        "settings_lock",
      );
    setCalibrationValues((current) => {
      ...
      wsClient.send(
        "save_settings",
        toRuntimeCalibrationPayload(runtimeValues),
      );
  ...
  const toggleOcrFilters = () => {
    if (isTranslating)
      return notify(
        "warning",
        "Çeviri aktifken filtre ayarları değiştirilemez",
        "settings_lock",
      );
    setOcrFiltersEnabled((current) => {
      const nextEnabled = !current;
      updateCalibrationProfileState({
        values: calibrationValues,
        ocrFiltersEnabled: nextEnabled,
      });
      const runtimeValues = currentRuntimeValues(
        calibrationValues,
        nextEnabled,
      );
      wsClient.send(
  ...
  const toggleRawTranslationFlow = () => {
    if (isTranslating)
      return notify(
        "warning",
        "Çeviri aktifken ham akış ayarı değiştirilemez",
        "settings_lock",
      );
    setRawTranslationFlowEnabled((current) => {
      const nextEnabled = !current;
      wsClient.send("save_settings", {
        raw_translation_flow_enabled: nextEnabled,
      });
      return nextEnabled;
    });
  };
  ...
  const saveCalibrationDraftProfile = () => {
    ...
    if (perfObj.isBase) {
      const existingCount = (
        ...
        (p) =>
          p.base_tier ===
          getSettingsPerformanceKeyFromConceptA(perfObj.baseTier),
      ).length;
      if (existingCount >= 2) {
        notify(
          "error",
          `Bu performans kademesi için en fazla 2 özel profil oluşturabilirsiniz.`,
        );
        return;
      }
    }
    const defaultName = perfObj.isBase
      ? `${perfObj.baseTier} (Özel)`
      : perfObj.name;
    setProfileNameInput(perfObj.isBase ? defaultName : perfObj.name);
    setProfileNameModal({
      open: true,
      mode: perfObj.isBase ? "create" : "rename",
      initialValue: perfObj.isBase ? defaultName : perfObj.name,
    });
  };
  ...
  const handleProfileModalSubmit = () => {
    ...
    if (profileNameModal.mode === "create") {
      ...
      const newProfile: AppCustomCalibrationProfile = {
        ...
        name: newName,
        mode: sceneType,
        base_tier: getSettingsPerformanceKeyFromConceptA(
          perfObj.baseTier,
        ) as AppPerformanceTier,
        values: {
          ...newSnapshot.values,
          ocr_filters_enabled: newSnapshot.ocrFiltersEnabled,
        },
      };
      wsClient.send("save_settings", {
      ...
      notify(
        "success",
        "Yeni performans profili başarıyla oluşturuldu ve kaydedildi.",
      );
      ...
      if (!perfObj.isBase) {
        ...
        if (calibrationDraftProfile?.id === perfObj.id) {
          setCalibrationDraftProfile((curr) =>
            curr
              ? {
                  ...curr,
                  name: newName,
  ...
  const deleteCalibrationDraftProfile = () => {
    ...
    if (!perfObj.isBase) {
      ...
      notify(
        "info",
        "Özel performans profili silindi. Varsayılan kademeye dönüldü.",
      );
    }
  };
  ...
  const resetCalibrationDraftProfile = () => {
    ...
    const resetSnapshot =
      calibrationDraftProfile?.savedSnapshot ?? initialSnapshot;
    ...
    setCalibrationDraftProfile((current) => {
      ...
      notify(
        "info",
        "Değişiklikler iptal edildi ve son kaydedilen duruma dönüldü.",
      );
      return {
        ...current,
  ...
  const activeCalibrationInfo =
    calibrationInfoContent[activeCalibrationInfoKey];
  const rawPreviewSrc = previewRegion?.image
    ? `data:image/png;base64,${previewRegion.image}`
    : null;
  const processedPreviewSrc = previewResult?.processed_image
    ? `data:image/png;base64,${previewResult.processed_image}`
    : null;
  const qualityScore =
    typeof previewResult?.quality_score === "number"
      ? previewResult.quality_score
      : null;
  const previewIssue =
    previewResult?.error ?? previewResult?.rejection_reason ?? null;
  const qualityText =
    previewResult?.detected_text ??
    (previewIssue ? `Önizleme sonucu: ${previewIssue}` : null);
    ...
    <>
        ...
        layers={[
          {
            inset: 0,
            background:
              "linear-gradient(180deg, rgba(5,9,14,0.98), rgba(3,6,10,1))",
          },
          {
            inset: "-10%",
            backgroundImage:
            ...
            maskImage:
              "radial-gradient(circle at center, rgba(0,0,0,0.88), transparent 84%)",
            opacity: 0.22,
          ...
          },
          {
            inset: "-20%",
            background:
              "linear-gradient(100deg, transparent 38%, rgba(90,200,255,0.18) 50%, transparent 62%)",
            filter: "blur(18px)",
      ...
      >
        ...
        >
          {calibrationInfoDockMounted ? (
            <CalibrationInfoDock
              info={activeCalibrationInfo}
              visible={calibrationInfoDockVisible}
            />
          ) : null}
          <div
            style={{
              ...
              >
                ...
                <SkeletonBlock
                  ...
                  headerAction={
                    <CalibrationHeaderActions
                      profileVisible={
                        Boolean(calibrationDraftProfile?.dirty) ||
                        !performanceList[safePerfIndex].isBase
                      }
                  ...
                  sample={
                    <div style={calibrationSettingsGridStyle}>
                      ...
                      >
                        ...
                        >
                          {(
                            ["sensitivity", "characters", "balance"] as const
                          ).map((key) => (
                            <CalibrationSliderControl
                              key={key}
                              ...
                              disabled={
                                !ocrFiltersEnabled &&
                                Boolean(
                                  calibrationControls[key]
                                    .dependsOnImageFilters,
                                )
                              }
                            />
                          ))}
                      ...
                      >
                        ...
                        <CalibrationGroupSection
                          ...
                          onInfoFocus={focusCalibrationInfo}
                        >
                          {(["bilateral", "gaussianC", "meanC"] as const).map(
                            (key) => (
                              <CalibrationSliderControl
                                key={key}
                                item={calibrationControls[key]}
                  ...
                  sample={
                    ...
                    >
                      <motion.button
                        type="button"
                        onClick={() => {
                          handleStartCalibrationSelect().catch(() => undefined);
                        }}
                      ...
                      <div
                        style={{
                          color: previewRegion
                            ? "rgba(223, 247, 255, 0.8)"
                            : "rgba(159, 183, 207, 0.45)",
                          padding: "8px 4px",
                          fontSize: 11.5,
                          fontWeight: 500,
                          lineHeight: 1.5,
                      ...
                      >
                        {previewRegion
                          ? "Canlı önizleme açık. Ayarları oynattıkça işlenmiş kare güncellenir."
                          : "Önce bir kalibrasyon alanı seç."}
                      </div>
                    </div>
                  }
                />
        ...
        ReactDOM.createPortal(
          ...
          >
            ...
            >
              ...
              >
                Teker &middot; zoom &nbsp;&nbsp;|&nbsp;&nbsp; Sürükle &middot;
                kaydır &nbsp;&nbsp;|&nbsp;&nbsp; ESC &nbsp;kapat
              </div>
            </div>
          </div>,
        ...
        >
          ...
          >
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") handleProfileModalSubmit();
                if (e.key === "Escape") setProfileNameModal(null);
              }}
            />
            <div
            ...
            >
              <button
                onClick={() => setProfileNameModal(null)}
                style={{
                  padding: "8px 16px",
