...
export const getLast = <T>(items: T[]) =>
  items.length > 0 ? items[items.length - 1] : undefined;

export const lastHardware = () =>
  (getLast(getEventHistory("hardware_result")) as HardwareResult | undefined) ??
  null;

export const lastSettings = () =>
  (getLast(getEventHistory("app_settings_loaded")) as
    | AppSettingsPayload
    | undefined) ?? {};

export const lastOfflineStatus = () =>
  (getLast(getEventHistory("offline_model_status")) as
    | OfflineStatusPayload
    | undefined) ?? null;

export const lastFrameStat = () =>
  (getLast(getEventHistory("ocr_frame_stat")) as
    | OcrFrameStatPayload
    | undefined) ?? null;

export const lastTranslationState = () =>
  (getLast(getEventHistory("translation_state")) as
    | TranslationStatePayload
    | undefined) ?? {};

export const normalizeOfflineStatus = (
  status: OfflineStatusPayload | null,
): OfflineStatusPayload | null => {
  if (!status) return null;
  const normalizedModelsReady =
    typeof status.models_ready === "object" && status.models_ready
      ? status.models_ready
      : {};
  const activeModel =
    typeof status.active_model === "string" && status.active_model
      ? status.active_model
      : typeof status.active_install_model === "string" &&
...
export const buildOfflineActionMap = (
  status: OfflineStatusPayload | null,
  ...
  if (status.active_model && status.active_action) {
    map[status.active_model] = {
      ...
      detail:
        status.detail ||
        (status.active_action === "remove"
          ? "Kaldırılıyor..."
          : "Kurulum sürüyor..."),
      stage:
        status.state ||
        (status.active_action === "remove" ? "remove" : "downloading"),
