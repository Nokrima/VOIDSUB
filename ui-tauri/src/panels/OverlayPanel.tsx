...
export const OverlayPanel: React.FC = () => {
  ...
  const activeProfileIndex =
    profiles.findIndex((p) => p.id === activeProfileId) >= 0
  ...
  const shiftProfile = (dir: -1 | 1) => {
    ...
    const nextIdx =
      (activeProfileIndex + dir + profiles.length) % profiles.length;
  ...
  useEffect(() => {
    if (isConnected && getEventHistory("overlay_settings_loaded").length === 0)
      send("get_settings");
    const u1 = onEvent("overlay_settings_loaded", (d) =>
      setCur({ ...(d as OverlaySettings), mode: "fixed" }),
    );
    const u2 = onEvent("app_settings_loaded", (d) =>
      setSpd(Number((d as AppSettingsLite).reading_speed_cps ?? 60)),
    );
  ...
  const updAnim = (v: TextAnim) => {
    setAnim(v);
    send("save_overlay_settings", { ...cur, anim: v, shadow });
  };
  const updShadow = (v: boolean) => {
    setShadow(v);
    send("save_overlay_settings", { ...cur, anim, shadow: v });
  };
  ...
  const upd = <K extends keyof OverlaySettings>(k: K, v: OverlaySettings[K]) =>
    setCur((p) => {
      if (p[k] === v) return p;
      const n = { ...p, [k]: v };
      send("save_overlay_settings", { ...n, anim, shadow });
      return n;
    });
  const updSpd = (v: number) => {
    const n = Math.min(90, Math.max(0, v));
    setSpd(n);
    send("save_settings", { reading_speed_cps: n });
  };
    ...
    <PanelStage
      ...
      layers={[
        ...
        },
        {
          ...
          opacity: 0.5,
          maskImage:
            "linear-gradient(180deg,transparent 8%,rgba(0,0,0,0.70) 30%,rgba(0,0,0,0.70) 70%,transparent 92%)",
          animation: "cBLine 6s linear infinite",
        },
        {
          inset: "-20%",
          background:
            "radial-gradient(circle at 22% 58%,rgba(251,191,36,0.14),transparent 34%),radial-gradient(circle at 76% 38%,rgba(56,130,220,0.12),transparent 30%),radial-gradient(circle at 50% 82%,rgba(168,85,247,0.08),transparent 26%)",
          filter: "blur(32px)",
          animation: "cBGlow 8s ease-in-out infinite",
        },
        {
          inset: "-20%",
          background:
            "linear-gradient(90deg,transparent 38%,rgba(251,191,36,0.10) 50%,transparent 62%)",
          filter: "blur(22px)",
          animation: "cBFlow 9s ease-in-out infinite",
        },
      ]}
    >
      <div
      ...
      >
        <div>
          ...
          <div style={pageSubStyle}>
            Ekran üzeri metin görünümünü yapılandır
          </div>
        </div>
        ...
        >
          ...
          >
              ...
              >
                <div
                  style={{
                    width: "100%",
                    display: "grid",
                    gridTemplateRows: "14px minmax(0, auto) 14px",
                    alignItems: "center",
                    justifyItems: "center",
                    gap: 2,
                    maxHeight: "100%",
            ...
            <Blk
              ...
              title={
                <div
                  key={cur.font_size}
                  style={{
                    animation: "cASwap 200ms ease both",
                    display: "flex",
                    alignItems: "center",
                    gap: 8,
                  }}
                >
                  ...
                  <span
                    style={{ fontSize: 12, color: "rgba(159,183,207,0.45)" }}
                  >
                    12 – 32 px
                  </span>
                </div>
            ...
            >
              <Bar
                ratio={(cur.font_size - 12) / 20}
                onDrag={(r) => upd("font_size", Math.round(12 + r * 20))}
              />
            </Blk>
            ...
            <Blk
              ...
              title={
                ...
                >
                  ...
                  >
                    {cur.font_bold ? "Kalın" : "Normal"}
                  </span>
                  <span
                    style={{ fontSize: 12, color: "rgba(159,183,207,0.45)" }}
                  >
                    {cur.font_bold ? "Ağır ağırlık" : "Standart ağırlık"}
                  </span>
                </div>
              }
            >
              <Toggle
                on={cur.font_bold}
                toggle={() => upd("font_bold", !cur.font_bold)}
                iOff="M4 7h16M4 12h16M4 17h16"
                iOn="M6 4h8a4 4 0 0 1 0 8H6ZM6 12h9a4 4 0 0 1 0 8H6Z"
              />
            </Blk>

            <Blk
              label="Yazı Gölgesi"
              icon="M12 8c-1.5 0-3 .8-3 2.5S10.5 16 12 16s3-1.5 3-4"
              title={
                <div
                  key={String(shadow)}
                  style={{
                ...
                >
                  ...
                  <span
                    style={{ fontSize: 12, color: "rgba(159,183,207,0.45)" }}
                  >
                    {shadow ? "Metin derinliği" : "Düz metin"}
                  </span>
                </div>
              }
            >
              <Toggle
                on={shadow}
                toggle={() => updShadow(!shadow)}
                iOff="M4 7h16M4 12h16M4 17h16"
                iOn="M3 12h18M6 8l-3 4 3 4M18 8l3 4-3 4"
              />
            </Blk>
          </div>
          ...
          >
            <Blk
              ...
              title={
                ...
                >
                  ...
                  >
                    {COLORS.find((c) => c.hex === cur.font_color)?.label ??
                      "Özel"}
                  </span>
                  <span
                    style={{ fontSize: 12, color: "rgba(159,183,207,0.45)" }}
                ...
                {COLORS.map(({ hex, label }) => (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow:
                        cur.font_color === hex
                          ? "0 0 0 2px rgba(125,211,252,0.70),0 0 12px rgba(125,211,252,0.20)"
              ...
              >
                {[0, 30, 45, 60, 90].map((v) => (
                  <button
                    ...
                    style={{
                      background:
                        spd === v ? "rgba(125,211,252,0.14)" : "transparent",
                      border: "none",
                      borderRadius: 6,
                      padding: "3px 6px",
                      color:
            ...
            <Blk
              ...
              title={
                ...
                >
                  ...
                  >
                    {cur.bg_visible ? "Görünür" : "Gizli"}
                  </span>
                  <span
                    style={{ fontSize: 12, color: "rgba(159,183,207,0.45)" }}
                  >
                    {cur.bg_visible ? "Bant modu aktif" : "Yalnızca metin"}
                  </span>
                </div>
              }
            >
              <Toggle
                on={cur.bg_visible}
                toggle={() => upd("bg_visible", !cur.bg_visible)}
                iOff="M21 3H3v18h18V3z"
                iOn="M3 3h18v18H3z"
              />
            </Blk>
            ...
            <Blk
              label="Saydamlık"
              icon="M12 3a9 9 0 1 0 0 18A9 9 0 0 0 12 3Z"
              title={
                <div
                  key={opa}
                  style={{
                ...
                >
                  ...
                  <span
                    style={{ fontSize: 12, color: "rgba(159,183,207,0.45)" }}
                  >
                    {cur.bg_visible ? "Arkaplan opaklığı" : "Arkaplan kapalı"}
                  </span>
                </div>
              }
              style={{
                opacity: cur.bg_visible ? 1 : 0.45,
                pointerEvents: cur.bg_visible ? "auto" : "none",
                transition: "opacity 200ms ease",
              }}
            >
              <Bar
                ratio={cur.alpha}
                onDrag={(r) => upd("alpha", Math.round(r * 100) / 100)}
              />
            </Blk>
          </div>
          ...
          >
            ...
            >
                ...
                >
                  <LayerHeaderActions
                    ...
                    onSave={() => {
                      if (activeProfile.isBase) {
                        if (profiles.length >= 6) {
                          notify(
                            "error",
                            "Maksimum 5 özel tasarım limitine ulaştınız.",
                            "concept_layer_action",
                          );
                          return;
                        }
                        ...
                        notify(
                          "success",
                          "Yeni tasarım profili başarıyla oluşturuldu.",
                          "concept_layer_action",
                        );
                      } else {
                        const newData = { cur, spd, shadow, anim };
                    ...
                    onDelete={() => {
                      if (!activeProfile.isBase) {
                        setProfiles((prev) =>
                          prev.filter((p) => p.id !== activeProfile.id),
                        );
                        ...
                        notify(
                          "info",
                          "Özel tasarım silindi. Varsayılan şablona dönüldü.",
                          "concept_layer_action",
                        );
                      } else {
                        ...
                        notify(
                          "info",
                          "Değişiklikler silindi. Varsayılan değerlere dönüldü.",
                          "concept_layer_action",
                        );
                      }
                    }}
                  ...
                  <button
                    ...
                    style={{
                      ...
                      filter: isPreviewActive
                        ? "drop-shadow(0 0 6px rgba(134,239,172,0.4))"
                        : "none",
                      transform: isPreviewActive ? "scale(1.1)" : "scale(1)",
                    }}
                  >
                    <G
                      p={
              ...
              <div style={TS}>
                Metin önizlemesi<span style={SS}>Gerçek konum yansıtılmaz</span>
              </div>
              <div
                style={{
                  flex: 1,
                  borderRadius: 14,
                  background:
                    "linear-gradient(180deg,rgba(4,8,13,0.72),rgba(2,5,9,0.86))",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  position: "relative",
              ...
              >
                <div
                  style={{
                    ...
                    inset: 0,
                    background:
                      "radial-gradient(circle at 50% 50%,rgba(251,191,36,0.06),transparent 55%)",
                    pointerEvents: "none",
                  }}
                />
                  ...
                  {anim === "slide" ? (
                    <>
                      ...
                      <div
                        style={{
                          ...
                          background: cur.bg_visible
                            ? `linear-gradient(180deg, rgba(12,16,24,${Math.max(cur.alpha, 0.05)}), rgba(4,8,14,${Math.max(cur.alpha, 0.05)}))`
                            : "transparent",
                          borderRadius: 18,
                          backdropFilter: cur.bg_visible
                            ? "blur(16px)"
                            : "none",
                          transition:
                      ...
                      >
                        <div
                          style={{
                            fontSize: cur.font_size,
                            fontFamily: cur.font_family,
                            color: cur.font_color,
                            fontWeight: cur.font_bold ? 800 : 500,
                    ...
                    <div
                      style={{
                        ...
                        transition:
                          anim === "none"
                            ? "background 200ms, padding 200ms, backdrop-filter 200ms"
                            : "opacity 500ms ease, transform 500ms cubic-bezier(0.2, 0.8, 0.2, 1), filter 500ms ease, background 200ms, padding 200ms, backdrop-filter 200ms",
                        opacity: isVisible || anim === "none" ? 1 : 0,
                        transform:
                          !isVisible && anim === "slide"
                    ...
                    >
                      ...
                      >
                        <ValueRail
                          ...
                          nextValue={
                            profiles[(activeProfileIndex + 1) % profiles.length]
                              .name
                          }
                        />
                      </div>
                ...
                >
                  {ANIMS.map((a) => (
                    <button
                      key={a.id}
                      type="button"
                      onClick={() => updAnim(a.id as TextAnim)}
                      title={a.label}
                      style={{
                        borderRadius: 8,
                        border: "none",
