...
export const EnginesPanel: React.FC = () => {
  ...
  const [selectedEngine, setSelectedEngine] = useState(
    initialSettings.ocr_engine ?? "easy",
  );
  const [isTranslating, setIsTranslating] = useState(
    Boolean(initialTranslationState.running),
  );
  const [offlineStatus, setOfflineStatus] =
    useState<OfflineStatusPayload | null>(
      normalizeOfflineStatus(lastOfflineStatus()),
    );
  const [frameStat, setFrameStat] = useState<OcrFrameStatPayload | null>(
    lastFrameStat,
  );
  const [modelActions, setModelActions] = useState<
    Record<string, OfflineModelAction>
  >(() => buildOfflineActionMap(normalizeOfflineStatus(lastOfflineStatus())));
  const actionStartRef = useRef<Record<string, number>>({});
  const [completedModelId, setCompletedModelId] = useState<string | null>(null);
  const [confirmRemoveLangId, setConfirmRemoveLangId] = useState<string | null>(
    null,
  );

  // EasyOCR Plugin State
  const [easyocrAction, setEasyocrAction] = useState<OfflineModelAction | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const offOfflineStatus = onEvent("offline_model_status", (data) => {
      ...
      setModelActions((current) => {
        ...
        return Object.keys(next).length > 0 || Object.keys(current).length === 0
          ? next
          : {};
      });
    });
    const offOfflineProgress = onEvent("offline_model_progress", (data) => {
      ...
      if (modelId) {
        if (!actionStartRef.current[modelId])
          actionStartRef.current[modelId] = Date.now();
        setModelActions((prev) => ({
          ...
          [modelId]: {
            ...
            stage,
            bytes_label: String(
              d.bytes_label ?? prev[modelId]?.bytes_label ?? "",
            ),
          },
        }));
      }
    ...
    const offFrameStat = onEvent("ocr_frame_stat", (data) =>
      setFrameStat(data as OcrFrameStatPayload),
    );

    // EasyOCR Plugin Events
    const offEasyProgress = onEvent(
    ...
    const offEasyError = onEvent(
      "easyocr_plugin_error",
      (data: { detail?: string }) => {
        setEasyocrAction(null);
    ...
    const offCudaComplete = onEvent("cuda_complete", () => {
      ...
      notify(
        "success",
        "CUDA başarıyla kuruldu! Aktif olması için VOIDSUB'ı yeniden başlatın.",
      );
    });
  ...
  const realHealthChecks: Record<string, HealthCheckItem[]> = {
    winonly: [
      {
        label: "Windows API",
        value: hardware?.available_engines.includes("winonly")
          ? "Hazır"
          : "Eksik",
        state: hardware?.available_engines.includes("winonly") ? "ok" : "error",
      },
      {
        label: "WinRT Capture",
        value: hardware?.winrt_available ? "Hazır" : "Eksik",
        state: hardware?.winrt_available ? "ok" : "error",
      },
      {
        label: "Motor Durumu",
        value:
          hardware?.engine_details?.winonly?.reason ||
          "Durum bilgisi bekleniyor",
        state: hardware?.available_engines.includes("winonly") ? "ok" : "warn",
      },
    ],
    easy: [
      {
        label: "CUDA",
        value: cudaRequiresRestart
          ? "Yeniden Başlatın"
          : hardware?.cuda_available
            ? "Aktif"
            : "Yok",
        state: cudaRequiresRestart
          ? "ok"
          : hardware?.cuda_available
            ? "ok"
            : "warn",
      ...
      {
        label: "Neural Ağlar",
        value: hardware?.available_engines.includes("easy")
          ? "Mevcut"
          : "Eksik",
        state: hardware?.available_engines.includes("easy") ? "ok" : "error",
      },
      {
        label: "Motor Durumu",
        value:
          hardware?.engine_details?.easy?.reason || "Durum bilgisi bekleniyor",
        state: hardware?.available_engines.includes("easy") ? "ok" : "warn",
      },
    ],
  };
    ...
    winonly: [
      {
        ...
        name: "Windows OCR",
        subtitle:
          hardware?.engine_details?.winonly?.reason || "Sistem OCR bileşeni",
        status: hardware?.available_engines.includes("winonly")
          ? "active"
          : "available",
      },
      {
        id: "w2",
        name: "WinRT Capture",
        subtitle: hardware?.winrt_available
          ? "Ekran yakalama hazır"
          : "Ekran yakalama eksik",
        status: hardware?.winrt_available ? "installed" : "available",
      },
    ],
    easy: [
      {
        ...
        name: "Easy Motoru",
        subtitle:
          hardware?.engine_details?.easy?.reason ||
          "OCR paketi kontrol ediliyor",
        status: hardware?.available_engines.includes("easy")
          ? "active"
          : "available",
      },
      {
        id: "m2",
        name: "CUDA Desteği",
        subtitle: cudaRequiresRestart
          ? "Aktif olması için uygulamayı yeniden başlatın"
          : hardware?.cuda_available
            ? "GPU hızlandırma açık"
            : "CPU moduna düşecek",
        status: cudaRequiresRestart
          ? "installed"
          : hardware?.cuda_available
            ? "installed"
            : "available",
  ...
  const realOfflineLangModels: OfflineLangModelItem[] = [
    {
      ...
      size:
        offlineStatus?.active_model === "opus_mt_en_tr" &&
        offlineStatus?.bytes_label
          ? offlineStatus.bytes_label
          : "Yerel model",
      status:
        offlineStatus?.selected_model === "opus_mt_en_tr" &&
        offlineStatus?.models_ready?.opus_mt_en_tr
          ? "active"
          : offlineStatus?.models_ready?.opus_mt_en_tr
            ? "installed"
            : "available",
    },
    {
      ...
      size:
        offlineStatus?.active_model === "nllb" && offlineStatus?.bytes_label
          ? offlineStatus.bytes_label
          : "Yerel model",
      status:
        offlineStatus?.selected_model === "nllb" &&
        offlineStatus?.models_ready?.nllb
          ? "active"
          : offlineStatus?.models_ready?.nllb
            ? "installed"
            : "available",
  ...
  const removeTargetModel = confirmRemoveLangId
    ? (realOfflineLangModels.find(
        (model) => model.id === confirmRemoveLangId,
      ) ?? null)
    : null;
  ...
  const handleLangDownload = (modelId: string) => {
    ...
    const activeModel =
      offlineStatus?.active_model ?? offlineStatus?.active_install_model;
    ...
    if (offlineStatus?.busy && activeModel && activeModel !== modelId) {
      injectInfoLog(
        "UI-011",
        `Başka yerel model işlemi sürüyor: ${activeModel}`,
      );
      send("get_offline_status");
      return;
    }
  ...
  const handleLangConfirmRemove = (modelId: string) => {
    ...
    if (offlineStatus?.busy) {
      injectInfoLog(
        "UI-012",
        `Kaldırma bekletildi; aktif işlem var: ${modelId}`,
      );
      return;
    }
    if (!offlineStatus?.models_ready?.[modelId]) {
      injectInfoLog(
        "UI-013",
        `Kaldırma atlandı; model kurulu değil: ${modelId}`,
      );
      setConfirmRemoveLangId(null);
      send("get_offline_status");
      return;
  ...
  const realPerfEstimate: Record<string, PerfEstimateItem> = {
    winonly:
      frameStat?.engine === "winonly"
        ? {
            fps: frameStat.result === "accepted" ? "Canlı" : "Beklemede",
            latency: `Kalite ${frameStat.quality}`,
            gpuUsage: hardware?.winrt_available ? "Düşük" : "--",
            fpsBar: frameStat.result === "accepted" ? 72 : 28,
            latencyBar: Math.max(10, Math.min(100, frameStat.quality)),
            gpuBar: 8,
          }
        : mockPerfEstimate.winonly,
    easy:
      frameStat?.engine === "easy"
        ? {
            fps: frameStat.result === "accepted" ? "Canlı" : "Beklemede",
            latency: `Kalite ${frameStat.quality}`,
            gpuUsage: hardware?.cuda_available ? "Aktif" : "CPU",
            fpsBar: frameStat.result === "accepted" ? 86 : 34,
            latencyBar: Math.max(10, Math.min(100, frameStat.quality)),
            gpuBar: hardware?.cuda_available ? 48 : 12,
          }
  ...
  const handleCudaDownload = () => {
    ...
    notify(
      "info",
      "CUDA indirmesi başlatılıyor (Bu işlem internet hızına göre zaman alabilir)...",
    );
    send("download_cuda");
  };
    ...
    <PanelStage
      css={`
        ...
        .item-feedback:hover {
          ...
          box-shadow:
            ...
            0 0 14px rgba(125, 211, 252, 0.05) !important;
        }
        .lang-row {
        ...
        .engine-card:hover {
          ...
          box-shadow:
            ...
            0 0 18px rgba(125, 211, 252, 0.06) !important;
        ...
        @keyframes completePulse {
          0% {
            box-shadow:
              inset 0 0 0 1px rgba(134, 239, 172, 0.6),
              0 0 10px rgba(134, 239, 172, 0.2);
          }
          50% {
            box-shadow:
              inset 0 0 0 1px rgba(134, 239, 172, 0.9),
      ...
      layers={[
        ...
        },
        {
          inset: 0,
          background:
            "radial-gradient(circle at 70% -20%, rgba(56,130,220,0.06), transparent 50%), radial-gradient(circle at -20% 120%, rgba(74,222,128,0.04), transparent 50%)",
        },
      ]}
    >
      ...
      <MotorDurumu
        ...
        onRefreshHardware={() => {
          ...
          injectInfoLog(
            "UI-015",
            "Motorları yeniden tarama isteği gönderildi.",
          );
        ...
        >
          <div
            ...
            style={{
              ...
              boxShadow:
                "0 26px 70px rgba(0,0,0,0.48), inset 0 1px 0 rgba(255,255,255,0.06)",
              padding: 22,
              position: "relative",
              overflow: "hidden",
            }}
          >
            <div
              style={{
                ...
                height: 1,
                background:
                  "linear-gradient(90deg, transparent, rgba(248,113,113,0.55), transparent)",
              }}
            />
            <div
              style={{
            ...
            >
              <strong style={{ color: "#fff" }}>
                {removeTargetModel.name}
              </strong>{" "}
              yerel diskten silinecek. Tekrar çevrimdışı kullanmak için modeli
              yeniden indirip dönüştürmen gerekecek.
            </div>
            <div
              style={{
                marginTop: 22,
            ...
            >
              ...
              <button
                type="button"
                disabled={Boolean(offlineStatus?.busy || isTranslating)}
                onClick={() => handleLangConfirmRemove(removeTargetModel.id)}
                style={{
                  ...
                  fontWeight: 800,
                  cursor:
                    offlineStatus?.busy || isTranslating
                      ? "not-allowed"
                      : "pointer",
                  opacity: offlineStatus?.busy || isTranslating ? 0.45 : 1,
                }}
              >
                Kaldır
              </button>
