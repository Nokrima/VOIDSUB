...
const toLogEntry = (data: {
  ...
  timestamp:
    String(data.timestamp ?? "--:--:--").substring(11, 23) ||
...
export const LogPanel: React.FC<{ embedded?: boolean }> = ({
  embedded = false,
}) => {
  const [logs, setLogs] = useState<LogEntry[]>(() =>
    getEventHistory("log_entry").map(toLogEntry),
  );
  ...
  useEffect(() => {
      ...
      const mockLogs: LogEntry[] = [
        {
          ...
          prefix: "CORE",
          code: "INIT",
          message: "Tercüment_v2 çekirdeği başlatılıyor...",
        },
        {
          id: "mock-2",
          timestamp: new Date(Date.now() - 14500)
            .toISOString()
            .substring(11, 23),
          level: "INFO",
          prefix: "SYS",
          code: "MEM_ALLOC",
          message: "Bellek yönetimi aktif (Max: 512MB)",
        },
        {
          id: "mock-4",
          timestamp: new Date(Date.now() - 12800)
            .toISOString()
            .substring(11, 23),
          level: "INFO",
          prefix: "WS",
          code: "READY",
          message: "İstemci bağlantısı başarıyla sağlandı.",
        },
        {
          id: "mock-5",
          ...
          code: "LOW_CONFIDENCE",
          message:
            "Seçili bölgedeki metin okunabilirliği düşük (%62). Aydınlatma koşullarını kontrol edin.",
        },
        {
          id: "mock-6",
          ...
          code: "REQ_TIMEOUT",
          message:
            "Çeviri servisine yapılan istek 5000ms içinde yanıt vermedi.",
        },
        {
          id: "mock-7",
          ...
          prefix: "API",
          code: "FALLBACK",
          message: "Yedek çeviri havuzuna geçiliyor...",
        },
      ];
      setLogs(mockLogs);
    }
    ...
    const unsubscribe = onEvent(
      ...
      (data: {
        ...
        if (
          lastLogRef.current &&
          lastLogRef.current.key === dedupeKey &&
          now - lastLogRef.current.at < 1800
        ) {
          return;
        }
        ...
        setLogs((prevLogs) => {
          ...
          return nextLogs.length > 500
            ? nextLogs.slice(nextLogs.length - 500)
            : nextLogs;
        });
  ...
  const knownPrefixes = useMemo(
    () => Array.from(new Set(logs.map((log) => log.prefix))).sort(),
    [logs],
  );
  const filteredLogs = logs.filter(
    (log) =>
      (filter === "ALL" || log.level === filter) &&
      (!prefixFilter || log.prefix === prefixFilter),
  );
  ...
  const handleCopy = (e: React.MouseEvent, id: string) => {
    ...
    const textToCopy = logsToCopy
      .map(
        (l) =>
          `[${l.timestamp}] [${l.level}] ${l.prefix !== "SYS" ? `[${l.prefix}] ` : ""}${l.code ? `[${l.code}] ` : ""}${l.message}`,
      )
      .join("\n");
  ...
  return (
    <div
      style={{
        ...
        background: embedded ? "transparent" : colors.panelBg,
        position: "relative",
        contain: embedded ? "layout paint style" : undefined,
      }}
    >
      ...
      <div
        ...
        className="log-panel-scroll app-glass"
        style={{
          ...glassCardStyle,
          flex: 1,
          minHeight: 0,
        ...
        >
          {filteredLogs.length === 0 ? (
            ...
            >
              ...
              >
                Bu seviyede gösterilecek kayıt yok...
              </div>
            </div>
          ) : (
            <div>
              {filteredLogs.map((log) => {
                ...
                const isLastSelected =
                  ...
                  selectedIds[selectedIds.length - 1] === log.id;
            ...
            >
              <span
                style={{
                  ...
                  height: 14,
                  background: "#7dd3fc",
                  animation: "conceptBlink 1s step-end infinite",
                }}
              />
            </div>
          )}
