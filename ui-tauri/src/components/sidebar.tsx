...
const MENU_ITEMS: { id: PageType; label: string; icon: string }[] = [
  { id: 'canvasA', label: 'Ana Ekran', icon: 'M3 10.5 12 3l9 7.5v9a1.5 1.5 0 0 1-1.5 1.5h-3.75V14.25h-7.5V21H4.5A1.5 1.5 0 0 1 3 19.5v-9Z' },
  { id: 'canvasC', label: 'Motorlar', icon: 'M10 3h4l1 2 2 1 2-1 2 4-1 2 1 2-2 4-2-1-2 1-1 2h-4l-1-2-2-1-2 1-2-4 1-2-1-2 2-4 2 1 2-1 1-2Zm2 6.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5Z' },
  { id: 'canvasB', label: 'Çeviri Katmanı', icon: 'M4 6.75A1.75 1.75 0 0 1 5.75 5h12.5A1.75 1.75 0 0 1 20 6.75v8.5A1.75 1.75 0 0 1 18.25 17H5.75A1.75 1.75 0 0 1 4 15.25v-8.5Zm3.5 12.25h9' },
  { id: 'canvasSettings', label: 'Ayarlar', icon: 'M12 3v3m0 12v3m9-9h-3M6 12H3m15.364 6.364-2.121-2.121M7.757 7.757 5.636 5.636m12.728 0-2.121 2.121M7.757 16.243l-2.121 2.121M12 8.5a3.5 3.5 0 1 0 0 7 3.5 3.5 0 0 0 0-7Z' },
];
...
export const Sidebar: React.FC<SidebarProps> = ({ activePage, onNavigate }) => {
  ...
  return (
    <aside
      ...
      style={{ width: expanded ? SIDEBAR_WIDTH.expanded : SIDEBAR_WIDTH.compact }}
    >
      <button
        type="button"
      ...
      <div className="flex flex-1 flex-col px-0">
        <div className="flex flex-1 flex-col gap-0">
          {MENU_ITEMS.map((item) => (
            <SidebarItem key={item.id} item={item} activePage={activePage} expanded={expanded} onNavigate={onNavigate} />
          ))}
        </div>
      </div>
    </aside>
