...
import { OpeningVariantStyles, openingVariants, type OpeningVariantKey } from './OpeningVariants';
...
export default function StartupIntro({
  ...
  const renderedProfileLines = profileLines.length > 0
    ? profileLines.slice(0, 3)
    : ['Profil Hazır', 'Varsayılan Mod', 'Easy • TR'];
  const sharedProfileBarWidth = useMemo(() => {
    const baseWidth = 154;
    const longestLineLength = Math.max(...renderedProfileLines.map((line) => line.length), 14);
    const maxExtraWidth = 28;
    const estimatedExtraWidth = Math.max(0, Math.min(maxExtraWidth, (longestLineLength - 14) * 2));
    return baseWidth + estimatedExtraWidth;
  }, [renderedProfileLines]);

  const stageInitial = mode === 'profileMode'
    ? { opacity: 0.76, filter: 'blur(1px)' }
    : { opacity: 0.72, scale: 0.982, filter: 'blur(2px)' };
  const stageAnimate = mode === 'profileMode'
    ? { opacity: [0.76, 1, 0.88], filter: ['blur(1px)', 'blur(0px)', 'blur(0.4px)'] }
    : { opacity: [0.72, 1, 0.86], scale: [0.982, 1.014, 1], filter: ['blur(2px)', 'blur(0px)', 'blur(0.6px)'] };
  ...
  return (
    <motion.div
      ...
      transition={{ duration: isExiting ? 0.32 : 0.42, ease: [0.22, 1, 0.36, 1] }}
    >
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <motion.div
          ...
          animate={{ x: [0, 24, -12, 0], y: [0, -14, 18, 0], scale: [1, 1.05, 0.98, 1] }}
        ...
        <motion.div
          ...
          animate={{ x: [0, -20, 16, 0], y: [0, 12, -16, 0], scale: [1, 0.96, 1.04, 1] }}
        ...
        <motion.div
          ...
          animate={{ x: [0, 18, -10, 0], y: [0, -18, 10, 0], scale: [1, 1.03, 0.97, 1] }}
      ...
      {!minimal ? (
        <motion.div
          ...
          transition={{ duration: hold ? 5.6 : 2.8, ease: 'easeOut', times: [0, 0.56, 1] }}
        >
          {mode === 'defaultMode' ? (
              ...
              <div className="absolute left-1/2 top-[22%] h-[132px] w-[316px] -translate-x-1/2">
                {[
                  ...
                  <motion.div
                    ...
                    animate={{ y: [0, -8, 0], rotate: [card.rotate, card.rotate + 4, card.rotate], opacity: [0.78, 1, 0.78] }}
            ...
            <div className="relative h-[300px] w-[560px]">
              <div className="absolute left-1/2 top-1/2 h-[188px] w-[356px] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-[28px] border border-white/8 bg-[rgba(255,255,255,0.03)] [backface-visibility:hidden] [transform:translate3d(-50%,-50%,0)]">
                <motion.div
                  ...
                  animate={{ scale: [0.992, 1.012, 1], y: [0, -2, 0], opacity: [0.92, 1, 0.94] }}
                  transition={{ duration: 2.8, ease: 'easeOut', times: [0, 0.58, 1] }}
                >
                  <motion.div
                    ...
                    style={{ background: `linear-gradient(135deg, ${variant.accent}, rgba(255,255,255,0.68))`, boxShadow: `0 18px 40px ${variant.accent}24` }}
                    animate={hasAvatar ? { y: [0, -5, 0], scale: [1, 1.025, 1], opacity: [0.9, 1, 0.9] } : { y: [0, -4, 0], opacity: [0.92, 1, 0.92] }}
                    transition={{ duration: 4.8, repeat: Infinity, ease: 'easeInOut' }}
                  >
                    {avatarDataUrl ? (
                      <img src={avatarDataUrl} alt={profileName ?? 'Profil'} className="h-full w-full rounded-full object-cover" draggable={false} />
                    ) : (
                      <motion.span
                        className="select-none text-[30px] font-semibold tracking-[-0.02em] [backface-visibility:hidden] [transform:translate3d(0,0,0)]"
                        animate={{ y: [0, -1.5, 0], opacity: [0.94, 1, 0.94], filter: ['blur(0.2px)', 'blur(0px)', 'blur(0.2px)'] }}
                        transition={{ duration: 4.8, repeat: Infinity, ease: 'easeInOut' }}
                      >
                        {profileInitial}
                      </motion.span>
                  ...
                  {renderedProfileLines.map((line, index) => {
                    ...
                    return (
                      <motion.div
                        ...
                        transition={{ delay: 0.46 + index * 0.14, duration: 0.55, ease: [0.22, 1, 0.36, 1] }}
                      >
                        <motion.div
                          className={`absolute left-1/2 top-1/2 h-[16px] -translate-x-1/2 -translate-y-1/2 rounded-full ${isMiddle ? '' : 'bg-white/10'}`}
                          style={{ width, background: isMiddle ? `${variant.accent}42` : undefined }}
                          initial={{ width: 6, opacity: 0.28, x: '-50%', y: '-50%', scaleX: 0.08 }}
                          animate={{
                            width,
                            opacity: isMiddle ? [0.38, 0.78, 0.54] : [0.22, 0.5, 0.32],
                            scaleX: 1,
                            y: ['-50%', isMiddle ? '-50%' : '-56%', '-50%'],
                          }}
                          transition={{ delay: 0.18 + index * 0.12, duration: 0.72, ease: [0.22, 1, 0.36, 1] }}
                        />
                        {!isMiddle ? (
                          <motion.div
                            className="absolute left-0 top-1/2 h-px w-full -translate-y-1/2 bg-white/12"
                            initial={{ opacity: 0, scaleX: 0.12 }}
                            animate={{ opacity: [0.12, 0.3, 0.18], scaleX: 1, y: [0, index === 0 ? -1.5 : 1.5, 0] }}
                            transition={{ delay: 0.28 + index * 0.12, duration: 0.82, ease: 'easeOut' }}
                          />
                        ) : null}
                        <motion.span
                          ...
                          initial={{ opacity: 0, filter: 'blur(10px)' }}
                          animate={{ opacity: 1, filter: 'blur(0px)' }}
                          transition={{ delay: 0.52 + index * 0.14, duration: 0.55, ease: [0.22, 1, 0.36, 1] }}
                        >
                          {line}
                        </motion.span>
                      </motion.div>
