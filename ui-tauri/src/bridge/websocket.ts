...
class TypedEventRegistry {
  private listeners: {
    [K in keyof WebSocketEventMap]?: Set<EventHandler<WebSocketEventMap[K]>>;
  } = {};

  public add<K extends keyof WebSocketEventMap>(
    event: K,
  ...
  public dispatch<K extends keyof WebSocketEventMap>(
    event: K,
...
const pushEventHistory = <K extends keyof WebSocketEventMap>(
  event: K,
  payload: WebSocketEventMap[K],
) => {
...
export const injectEvent = <K extends keyof WebSocketEventMap>(
  event: K,
  payload: WebSocketEventMap[K],
) => {
...
export const getEventHistory = <K extends keyof WebSocketEventMap>(
  event: K,
): WebSocketEventMap[K][] => {
  ...
  switch (eventName) {
    ...
    case "hardware_result":
      return (
        ...
        "gpu" in payload &&
        "ram_gb" in payload &&
        Array.isArray(payload.available_engines)
      );
    ...
    case "temporary_region_selected":
      return (
        "x1" in payload && "y1" in payload && "x2" in payload && "y2" in payload
      );
  ...
  socket.onmessage = (event) => {
    try {
      ...
      if (typeof payload !== "object" || payload === null) {
        console.warn(
          `[WS] Geçersiz payload formatı (${eventName}). Nesne bekleniyordu.`,
        );
        return;
      }
      ...
      if (!validatePayload(eventName, payload as Record<string, unknown>)) {
        console.warn(
          `[WS] Runtime doğrulama hatası. Olay: ${eventName}, Payload:`,
          payload,
        );
        return;
      }
...
export const send = <K extends keyof WebSocketEventMap>(
  event: K,
  ...
  if (
    ...
    if (pendingMessages.length >= MAX_PENDING) {
      console.warn(
        "[WS] Pending queue dolu (%d/%d), mesaj atlandı: %s",
...
export const onEvent = <K extends keyof WebSocketEventMap>(
  event: K,
  handler: EventHandler<WebSocketEventMap[K]>,
) => {
