...
async def test_pipeline_mixin_smoke():
    ...
    pipeline.translation_queue._translate_with_engine = MagicMock(
        return_value=("Merhaba dünya", "google")
    )
    pipeline.translation_queue._select_translation_result = MagicMock(
        return_value=("Merhaba dünya", "google")
    )
    pipeline.translation_queue._translate_text = MagicMock(
        return_value=("Merhaba dünya", "google")
    )
    ...
    translation_call = [
        call
        for call in bridge_mock.send.mock_calls
        if call.args[0] == "new_translation"
    ][0]
