...
def test_easyocr_bad_zip_error(manager, mock_bridge):
    ...
    manager._extract_plugin = MagicMock(
    ...
    error_calls = [
        c for c in mock_bridge.send.mock_calls if c.args[0] == "easyocr_plugin_error"
    ]
...
def test_easyocr_disk_error(manager, mock_bridge):
    ...
    manager._extract_plugin = MagicMock(
        side_effect=RuntimeError("Zip cikartma hatasi: No space left on device")
    )
