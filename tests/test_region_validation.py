...
def test_bridge_normalize_region():
    ...
    with patch("ctypes.windll.user32.GetSystemMetrics", side_effect=mock_get_system_metrics):
        # Test normal region
        res = bridge._normalize_region({"left": 100, "top": 100, "width": 500, "height": 300})
...
def test_capture_resolve_region():
    ...
    with patch.object(capturer, "_virtual_bounds", return_value={"left": 0, "top": 0, "width": 1920, "height": 1080}):
        res = capturer.resolve_region({"left": -100, "top": -100, "width": 4000, "height": 3000})
        assert res["left"] == 0
        assert res["top"] == 0
