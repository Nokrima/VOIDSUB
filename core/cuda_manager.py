...
class CudaManager:
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "detail": "CUDA hızlandırma modülü hazır."
    ...
    def start(self, bridge=None) -> None:
        ...
        if self._thread and self._thread.is_alive():
            log_event(
                PREFIX_SYS,
                "091",
                "[CUDA Kurulumu] -> REDDEDİLDİ | Kurulum zaten sürüyor",
                level="warning",
            )
            self._send_status()
            return
        ...
        self._thread = threading.Thread(
            target=self._install_worker, name="cuda-install-worker", daemon=True
        )
        self._thread.start()
    ...
    def remove(self) -> None:
        ...
        self._cancel.clear()
        self._thread = threading.Thread(
            target=self._remove_worker, name="cuda-remove-worker", daemon=True
        )
        self._thread.start()
    ...
    def _install_worker(self) -> None:
        ...
        try:
            ...
            if is_compiled:
                raise RuntimeError(
                    "Derlenmiş sürümde CUDA doğrudan pip ile kurulamaz. Lütfen ayarlardan EasyOCR eklentisini indirin."
                )
            ...
            log_event(
                PREFIX_SYS,
                "094",
                f"[CUDA Kurulumu] -> TAMAMLANDI | Süre(ms): {elapsed_ms}",
            )
            self._send_status()
    ...
    def _remove_worker(self) -> None:
        ...
        try:
            ...
            self._set_stage(
                "remove", 10, "CUDA kütüphaneleri sistemden kaldırılıyor..."
            )
            ...
            log_event(
                PREFIX_SYS,
                "099",
                f"[CUDA Kaldırma] -> TAMAMLANDI | Süre(ms): {elapsed_ms}",
            )
            self._send_status()
    ...
    def _run_process(self, command: list[str], failure_message: str) -> None:
                    ...
                    if is_stdout:
                        ...
                        if "Downloading" in line_str or "Downloading torch" in line_str:
                            self._set_stage(
                                "downloading",
                                20,
                                "Dosyalar indiriliyor (Bağlantı hızına göre sürebilir)",
                            )
                            m = re.search(
                                r"\(([\d.]+\s*[MkmG]B)\)", line_str, re.IGNORECASE
                            )
                            if m:
                                self.bytes_label = f"~ {m.group(1)}"

                            self.detail = line_str[:60] + (
                                "..." if len(line_str) > 60 else ""
                            )
                            self._send_progress()
                            ...
                            self._set_stage(
                                "installing",
                                80,
                                "Sisteme entegre ediliyor (1-2 dakika sürebilir)...",
                            )
        ...
        t_out = threading.Thread(
            target=_read_stream, args=(self.active_proc.stdout, True), daemon=True
        )
        t_err = threading.Thread(
            target=_read_stream, args=(self.active_proc.stderr, False), daemon=True
        )
        t_out.start()
        t_err.start()
        ...
        if self.active_proc.returncode != 0:
            for line in list(stderr_deque):
                log_event(
                    PREFIX_SYS, "096", f"[CUDA Kurulum Hatası] -> {line}", level="error"
                )
            raise RuntimeError(
    ...
    def _fail(self, message: str) -> None:
        ...
        log_error(
            PREFIX_SYS,
            "097",
            f"[CUDA Kurulumu] -> BAŞARISIZ | Süre: {elapsed_ms} | Hata: {message}",
            "CUDA indirilemedi.",
        )
