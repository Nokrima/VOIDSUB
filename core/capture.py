...
def _load_winrt_runtime() -> bool:
    ...
    try:
        ...
        from winrt.windows.graphics.directx import (
            DirectXPixelFormat as _DirectXPixelFormat,
        )
        ...
        if getattr(sys, "frozen", False) or "__compiled__" in globals():
            ...
            overlay_root = (
                Path(__file__).resolve().parent / "ocr" / "_winrt_overlay" / "winrt"
            )
        ...
        create_direct3d11_device_from_dxgi_device = (
            _create_direct3d11_device_from_dxgi_device
        )
...
class ScreenCapturer:
    ...
    def start_camera(self) -> bool:
        ...
        try:
            ...
            return False
        self.logger.info(
            f"[{PREFIX_SYS}-075] WGC start requested: supported={supported}"
        )
        if not supported:
            ...
            log_error(
                PREFIX_SYS,
                "014",
                "Windows Graphics Capture unsupported",
                "WGC bu sistemde desteklenmiyor.",
            )
            return False
        ...
        self._capture_thread = threading.Thread(
            target=self._capture_worker, name="wgc-capture", daemon=True
        )
    ...
    def capture_region(self, region: dict) -> Optional[np.ndarray]:
        if not region or self._backend != "wgc":
            self.logger.debug(
                f"[{PREFIX_SYS}-076] Capture skipped: backend={self._backend}, region={region}"
            )
            return None
        ...
        with self._frame_lock:
            ...
            frame_age = (
                time.monotonic() - self._latest_frame_time
        ...
        if frame is None:
            self._set_capture_state("unavailable")
            self.logger.debug(
                f"[{PREFIX_SYS}-077] Capture unavailable: resolved={resolved}, bounds={bounds}, frame_seq={frame_seq}"
            )
            return None
        ...
        inset_left, inset_top, inset_right, inset_bottom = self._capture_inset(
            width, height
        )
        left += inset_left
        top += inset_top
    ...
    def capture_virtual_desktop_png_data_url(self) -> Optional[str]:
        ...
        with self._frame_lock:
            frame = (
                None
                if self._latest_frame_bgra is None
                else self._latest_frame_bgra.copy()
            )
        if frame is None:
            return None
        try:
            ...
            log_error(
                PREFIX_SYS, "015", str(exc), "Secim onizleme goruntusu alinamadi."
            )
            return None
    ...
    def resolve_region(self, region: dict) -> dict:
        ...
        next_region["left"] = max(
            v_left, min(int(next_region.get("left", 0)), v_right - 10)
        )
        next_region["top"] = max(
            v_top, min(int(next_region.get("top", 0)), v_bottom - 10)
        )
        next_region["width"] = max(
            10, min(int(next_region.get("width", 0)), v_right - next_region["left"])
        )
        next_region["height"] = max(
            10, min(int(next_region.get("height", 0)), v_bottom - next_region["top"])
        )

        self._last_resolved_region = (
            dict(next_region) if isinstance(next_region, dict) else None
        )
        return next_region

    def get_last_resolved_region(self) -> Optional[dict]:
        return (
            dict(self._last_resolved_region)
            if isinstance(self._last_resolved_region, dict)
            else None
        )
    ...
    def refresh_region_anchor(self, region: dict) -> dict:
        ...
        if not isinstance(follow_window, dict):
            self.logger.debug(
                f"[{PREFIX_SYS}-060] Region refresh bypassed: no_follow_window region={region}"
            )
            return dict(region)
        resolved = self._resolve_window_region(follow_window)
        if not resolved:
            self.logger.debug(
                f"[{PREFIX_SYS}-061] Region refresh failed: follow_window={follow_window}, using_saved_region={region}"
            )
            return dict(region)
    ...
    def _capture_inset(self, width: int, height: int) -> tuple[int, int, int, int]:
        if width <= 0 or height <= 0:
            return (0, 0, 0, 0)
        side_inset = min(
            self._CAPTURE_INSET_SIDE_MAX, int(width * self._CAPTURE_INSET_SIDE_RATIO)
        )
        top_inset = min(
            self._CAPTURE_INSET_TOP_MAX, int(height * self._CAPTURE_INSET_TOP_RATIO)
        )
        bottom_inset = min(
            self._CAPTURE_INSET_BOTTOM_MAX,
            int(height * self._CAPTURE_INSET_BOTTOM_RATIO),
        )
    ...
    def _capture_worker(self) -> None:
        ...
        try:
            if (
                GraphicsCaptureAccess is None
                or Direct3D11CaptureFramePool is None
                or GraphicsCaptureItem is None
            ):
                raise RuntimeError("WinRT sembolleri yuklenemedi (globals None).")
            winrt_device = self._create_winrt_device()
            access = GraphicsCaptureAccess.request_access_async(
                ...
                with frame:
                    ...
                    try:
                        if bitmap.bitmap_pixel_format != BitmapPixelFormat.BGRA8:
                            converted = SoftwareBitmap.convert_with_alpha(
                                bitmap,
                        ...
                        pixels = np.frombuffer(bytes(buffer), dtype=np.uint8).reshape(
                            (bitmap.pixel_height, bitmap.pixel_width, 4)
                        )
                        with self._frame_lock:
    ...
    def _check_capture_health(self) -> None:
        ...
        log_error(
            PREFIX_SYS,
            "012",
            self._runtime_error or "worker_stopped",
            "WGC worker durdu.",
        )
    ...
    def _create_winrt_device(self):
        ...
        hr = query_interface(
            unknown, ctypes.byref(IID_IDXGIDevice), ctypes.byref(dxgi_device)
        )
    ...
    def _resolve_window_region(self, follow_window: dict) -> Optional[dict]:
        try:
            ...
            if hwnd <= 0 or not user32.IsWindow(hwnd):
                self.logger.debug(
                    f"[{PREFIX_SYS}-063] Window region resolve: no_valid_hwnd follow_window={follow_window}"
                )
                return None
            rect = ctypes.wintypes.RECT()
            if not user32.GetWindowRect(hwnd, ctypes.byref(rect)):
                self.logger.debug(
                    f"[{PREFIX_SYS}-064] Window region resolve: GetWindowRect failed hwnd={hwnd}"
                )
                return None
            result = {
            ...
            self.logger.debug(
                f"[{PREFIX_SYS}-065] Window region resolve: hwnd={hwnd}, result={result}"
            )
            return result
        except Exception as exc:
            log_error(
                ...
                "016",
                str(exc),
                "Pencere bagli yakalama bolgesi cozulurken hata olustu.",
            )
            return None
    ...
    def _resolve_follow_window_handle(self, follow_window: dict) -> int:
        ...
        if hwnd > 0 and user32.IsWindow(hwnd):
            if self._window_matches_metadata(hwnd, follow_window):
                self.logger.debug(
                    f"[{PREFIX_SYS}-066] Follow window handle reused: hwnd={hwnd}, follow_window={follow_window}"
                )
                return hwnd
            self.logger.debug(
                f"[{PREFIX_SYS}-067] Follow window handle mismatch: hwnd={hwnd}, follow_window={follow_window}"
            )
        rebound = self._find_matching_window(follow_window)
        self.logger.debug(
            f"[{PREFIX_SYS}-068] Follow window rebound: old_hwnd={hwnd}, rebound_hwnd={rebound}, follow_window={follow_window}"
        )
    ...
    def _build_follow_window_metadata(
        self, hwnd: int, follow_window: dict, rect: ctypes.wintypes.RECT
    ) -> dict:
    ...
    def _get_window_metadata(self, hwnd: int) -> Optional[dict]:
        try:
            ...
            log_error(
                ...
                "019",
                str(exc),
                f"Pencere metadatası alınırken hata (hwnd: {hwnd}).",
            )
            return None
