...
class PerformanceMonitor:
    ...
    def __init__(
        self, bridge: Any, pipeline: Any, sample_interval: float = 1.0
    ) -> None:
        ...
        self._last_gpu_stats: dict[str, int | None] = {
            "util_percent": None,
            "vram_used_mb": None,
        }
        try:
            ...
            self._psutil = None
            self.logger.warning(
                f"[{PREFIX_SYS}-080] Performance monitor psutil hazir degil: {exc}"
            )
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._worker, name="performance-monitor", daemon=True
        )
        self._thread.start()
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        if self._process is not None:
            try:
                ...
                ram_mb = round(
                    float(self._process.memory_info().rss) / (1024 * 1024), 1
                )
                thread_count = int(self._process.num_threads())
            except Exception as exc:
                self.logger.debug(
                    f"[{PREFIX_SYS}-083] Process statistics fetch failed: {exc}"
                )
        ...
        return {
            ...
            "pipeline": {
                ...
                "translation_engine": str(
                    getattr(self.pipeline, "translation_engine", "")
                ),
                "tier": str(getattr(self.pipeline, "performance_tier", "")),
                "pending_translations": len(
                    getattr(self.pipeline, "_pending_translations", [])
                ),
                "latest_frame_id": int(getattr(self.pipeline, "_latest_frame_id", 0)),
                "processed_frame_id": int(
                    getattr(self.pipeline, "_processed_frame_id", 0)
                ),
                "reused_frame_count": int(
                    getattr(self.pipeline, "_reused_frame_count", 0)
                ),
                "capture_state": str(
                    getattr(
                        getattr(self.pipeline, "capturer", None),
                        "_capture_state",
                        "unknown",
                    )
                ),
                "capture_ms": round(float(latest_probe.get("capture_ms", 0.0)), 1)
                if latest_probe
                else 0.0,
                "frame_shape": list(latest_probe.get("shape", ()))
                if latest_probe
                else [],
                "last_latency_ms": round(
                    float(last_perf.get("frame_to_overlay_ms", 0.0)), 1
                )
                if last_perf
                else 0.0,
                "last_ocr_ms": round(float(last_perf.get("ocr_ms", 0.0)), 1)
                if last_perf
                else 0.0,
                "last_translation_ms": round(
                    float(last_perf.get("translation_ms", 0.0)), 1
                )
                if last_perf
                else 0.0,
                "dynamic_capture_delay_ms": round(
                    float(getattr(self.pipeline, "_capture_delay", lambda: 0.0)())
                    * 1000,
                    1,
                ),
            },
    ...
    def _worker(self) -> None:
        if self._process is not None:
            try:
                ...
                self.logger.debug(
                    f"[{PREFIX_SYS}-084] Process initial cpu_percent failed: {exc}"
                )
        while not self._stop_event.wait(self.sample_interval):
            try:
                ...
                self.logger.debug(
                    f"[{PREFIX_SYS}-082] Performance sample emit atlandi: {exc}"
                )
    ...
    def _read_gpu_stats(self) -> dict[str, int | None]:
        ...
        try:
            ...
            self.logger.debug(
                f"[{PREFIX_SYS}-086] nvidia-smi stats fetch failed: {exc}"
            )
        return dict(self._last_gpu_stats)
