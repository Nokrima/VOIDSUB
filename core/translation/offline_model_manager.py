...
class OfflineModelManager:
    def __init__(
        self, models_dir: Path, model_key: str = DEFAULT_MODEL_KEY, bridge=None
    ):
                    ...
                    try:
                        ...
                        if age_hours > 24:
                            ...
                            log_event(
                                PREFIX_TRL,
                                "073",
                                f"[Sistem Temizliği] -> SİLİNDİ | Bayat tmp klasörü: {d.name}",
                                level="warning",
                            )
                    except Exception:
                        pass
    ...
    def set_model_key(self, model_key: str) -> None:
        ...
        self.tmp_dir = (
            self.models_dir / f"{self.spec['folder']}{self.spec['tmp_suffix']}"
        )
    ...
    def verify_integrity(self) -> bool:
        ...
        try:
            ...
            for name, expected_hash in expected.items():
                ...
                if not file_path.exists():
                    log_error(
                        PREFIX_TRL,
                        "090",
                        f"[Model Bütünlüğü] -> EKSİK DOSYA | Detay: {name} bulunamadı",
                        "Model dosyasi silinmis!",
                    )
                ...
                if md5.hexdigest() != expected_hash:
                    log_error(
                        PREFIX_TRL,
                        "091",
                        f"[Model Bütünlüğü] -> BOZUK DOSYA | Detay: {name} (MD5 uyuşmazlığı)",
                        "Model dosyasi bozulmus (MD5 uyusmazligi)!",
                    )
                    self._handle_corrupt_model()
                    return False
            return True
            ...
            log_error(
                PREFIX_TRL,
                "092",
                f"[Model Bütünlüğü] -> KONTROL HATASI | Hata: {exc}",
                "Model butunlugu dogrulanamadi.",
            )
            self._handle_corrupt_model()
            return False
    ...
    def _handle_corrupt_model(self) -> None:
        ...
        log_event(
            PREFIX_TRL,
            "060",
            f"[Model Yöneticisi] Model bozuk/eksik tespit edildi, yeniden indirme başlatılıyor: {self.model_key}",
        )
        emit_bridge_event(
            "log_entry",
            {
    ...
    def recover(self) -> None:
        ...
        if self.model_dir.exists() and not self._is_ready():
            ...
            log_event(
                PREFIX_TRL,
                "061",
                f"[Sistem Temizliği] -> TEMİZLENDİ | Eksik runtime klasörü: {self.model_dir}",
                level="warning",
            )
        self._reset_idle_state()
        self._send_status()
    ...
    def start(self, bridge=None) -> None:
        ...
        if self._is_ready():
            log_event(
                PREFIX_TRL,
                "041",
                f"[Model Kurulumu] -> ATLANDI | Model zaten hazır: {self.model_key}",
            )
            self._send_status()
            return
        if self._thread and self._thread.is_alive():
            log_event(
                PREFIX_TRL,
                "042",
                f"[Model Kurulumu] -> REDDEDİLDİ | Kurulum zaten sürüyor: {self.model_key}",
                level="warning",
            )
            self._send_status()
            return
        ...
        self._thread = threading.Thread(
            ...
            name=f"offline-model-install-{self.model_key}",
            daemon=True,
        )
        self._thread.start()
    ...
    def cancel(self) -> None:
        ...
        log_event(
            PREFIX_TRL,
            "056",
            f"[Model Kurulumu] -> İPTAL EDİLDİ | Model: {self.model_key}",
            level="warning",
        )
        self._send(
            "offline_model_cancelled",
            {
                "message": f"{self.spec['label']} kurulumu iptal edildi.",
                "model": self.model_key,
            },
        )
        self._send_status()
    ...
    def pause(self) -> None:
        ...
        log_event(
            PREFIX_TRL,
            "056",
            f"[Model Kurulumu] -> DURAKLATILDI | Model: {self.model_key}",
            level="info",
        )
        self._send(
            "offline_model_paused",
            {
                "message": f"{self.spec['label']} kurulumu duraklatıldı.",
                "model": self.model_key,
            },
        )
        self._send_status()
    ...
    def remove(self) -> None:
        ...
        log_event(
            PREFIX_TRL,
            "054",
            f"[Model Kaldırma] -> BAŞLADI | Model: {self.model_key} | Klasör: {self.model_dir}",
        )
        self._cancel.set()
        self._pause.clear()
        if self.active_proc and self.active_proc.poll() is None:
        ...
        log_event(
            PREFIX_TRL,
            "055",
            f"[Model Kaldırma] -> TAMAMLANDI | Model: {self.model_key} | Süre(ms): {elapsed_ms}",
        )
        self._send_status()
    ...
    def save_runtime_profile(self, profile: dict) -> None:
        target = (
            ...
            else self.pending_profile_path
        )
        ...
        for candidate in (self.runtime_profile_path, self.pending_profile_path):
            ...
            try:
                ...
                log_event(
                    PREFIX_TRL,
                    "078",
                    f"Runtime profili okunamadi: model={self.model_key} path={candidate} error={exc}",
                    level="warning",
                )
        return None
    ...
    def _install_worker(self) -> None:
        ...
        try:
            ...
            log_event(
                PREFIX_TRL,
                "040",
                f"[Model Kurulumu] -> BAŞLADI | Model: {self.model_key} | Repo: {self.spec['repo_id']}",
            )
            ...
            log_event(
                PREFIX_TRL,
                "059",
                f"[Model Kurulumu] -> TAMAMLANDI | Model: {self.model_key} | Süre(ms): {elapsed_ms}",
            )
            self._send_status()
            self._send(
                "offline_model_complete",
                {"percent": 100, "model": self.model_key, "elapsed_ms": elapsed_ms},
            )
    ...
    def _ensure_packages(self) -> None:
        ...
        if is_compiled:
            ...
            if not plugin_python:
                raise RuntimeError(
                    "Çevrimdışı modelleri indirebilmek ve dönüştürebilmek için önce Ayarlar'dan EasyOCR Eklentisini indirmeniz gerekmektedir (PyTorch barındırır)."
                )
            # Plugin içinde transformers ve ctranslate2 olduğundan emin olalım
            self._install_package_args(
                [
                    "ctranslate2",
                    "transformers",
                    "sentencepiece",
                ...
                "Eklenti içine dönüştürücü paketleri kurulamadı",
            )
            return
        ...
        log_event(
            PREFIX_TRL,
            "043",
            f"[Paket Yöneticisi] -> KONTROL EDİLDİ | Model: {self.model_key} | Eksik: {missing} | Torch: {torch_ready}",
        )
        if missing:
            self._install_package_args([*missing], "Temel Python paketleri kurulamadı")
        if not torch_ready:
            self._install_package_args(
                ["torch>=2.6"], "PyTorch kurulumu/güncellemesi tamamlanamadı"
            )
    ...
    def _install_package_args(self, packages: list[str], failure_message: str) -> None:
        log_event(
            PREFIX_TRL,
            "044",
            f"[Paket Yöneticisi] -> KURULUM BAŞLADI | Paketler: {packages}",
        )
        ...
        log_event(
            PREFIX_TRL,
            "045",
            f"[Paket Yöneticisi] -> KURULUM TAMAMLANDI | Paketler: {packages}",
        )
    ...
    def _prepare_workspace(self) -> None:
        ...
        log_event(
            PREFIX_TRL,
            "046",
            f"[Çalışma Alanı] -> HAZIRLANDI | Geçici Klasör: {self.tmp_dir}",
        )
    ...
    def _build_download_plan(self) -> list[dict]:
        ...
        weight_file = next(
            (name for name in self.spec["preferred_weight_files"] if name in siblings),
            None,
        )
        if not weight_file:
            raise RuntimeError(
                f"Agirlik dosyasi bulunamadi: {self.spec['preferred_weight_files']}"
            )
    ...
    def _download_files(self, files: list[dict]) -> None:
        ...
        for index, item in enumerate(files, start=1):
            ...
            self._set_stage(
                "downloading", percent, f"{name} indiriliyor... ({index}/{len(files)})"
            )
            log_event(
                PREFIX_TRL,
                "048",
                f"[Model İndirme] -> İNDİRİLİYOR | Dosya: {name} ({index}/{len(files)}) | Boyut: {self._format_bytes(size) if size else 'Bilinmiyor'}",
            )
            started_at = time.monotonic()
            try:
                ...
                log_error(
                    PREFIX_TRL,
                    "049",
                    f"[Model İndirme] -> İNDİRME BAŞARISIZ | Model: {self.model_key} | Dosya: {name} | Hata: {exc}",
                    "Offline model dosyasi indirilemedi.",
                )
                raise
            elapsed_ms = int((time.monotonic() - started_at) * 1000)
            resolved = self.tmp_dir / name
            downloaded += (
                size if size else (resolved.stat().st_size if resolved.exists() else 0)
            )
            self.bytes_label = f"{self._format_bytes(min(downloaded, total))} / {self._format_bytes(total)}"
            log_event(
                PREFIX_TRL,
                "050",
                f"[Model İndirme] -> İNDİRİLDİ | Dosya: {name} | Süre(ms): {elapsed_ms}",
            )
        ...
        while True:
            try:
                ...
                log_event(
                    PREFIX_TRL,
                    "051",
                    "[Ağ Bağlantısı] -> BEKLENİYOR | İnternet koptu, yeniden deneniyor...",
                    level="warning",
                )
    ...
    def _verify_download_plan(self, files: list[dict]) -> None:
        missing = [
            str(item["name"])
            for item in files
            if not (self.tmp_dir / str(item["name"])).exists()
        ]
        if missing:
        ...
        log_event(
            PREFIX_TRL,
            "052",
            f"[Model İndirme] -> DOĞRULANDI | Toplam Dosya: {len(files)}",
        )
    ...
    def _convert_model(self) -> None:
        ...
        copy_files = [
            name
            for name in self.spec["tokenizer_files"]
            if (self.tmp_dir / name).exists()
        ]
        ...
        log_event(
            PREFIX_TRL,
            "053",
            f"[Model Dönüşümü] -> BAŞLADI | Model: {self.model_key} | Komut: {' '.join(command)}",
        )
        started_at = time.monotonic()
        try:
            ...
            log_error(
                PREFIX_TRL,
                "057",
                f"[Model Dönüşümü] -> DÖNÜŞÜM BAŞARISIZ | Model: {self.model_key} | Hata: {exc}",
                "Offline model donusumu tamamlanamadi.",
            )
            raise
        elapsed_ms = int((time.monotonic() - started_at) * 1000)
        log_event(
            PREFIX_TRL,
            "058",
            f"[Model Dönüşümü] -> TAMAMLANDI | Süre(ms): {elapsed_ms}",
        )
    ...
    def _copy_tokenizer_files(self) -> None:
        ...
        for name in tuple(self.spec["tokenizer_files"]):
            ...
            if source.exists() and not target.exists():
                ...
                log_event(
                    PREFIX_TRL, "062", f"[Model Dönüşümü] -> KOPYALANDI | Dosya: {name}"
                )
    ...
    def _verify_model(self) -> None:
        ...
        missing = [
            name
            for name in tuple(self.spec["ready_files"])
        ...
        log_event(
            PREFIX_TRL,
            "063",
            f"[Model Doğrulama] -> DOĞRULANDI | İstenen Dosyalar: {self.spec['ready_files']}",
        )
        import hashlib
        ...
        if self.spec["runtime_kind"] == "opus":
            ...
            tokenizer = AutoTokenizer.from_pretrained(
                str(self.model_dir), src_lang="eng_Latn", use_fast=False
            )
        ...
        log_event(
            PREFIX_TRL,
            "064",
            f"[Model Doğrulama] -> BAŞARILI & MD5 OLUŞTURULDU | Model: {self.model_key}",
        )
    ...
    def _promote_pending_runtime_profile(self) -> None:
        ...
        log_event(
            PREFIX_TRL,
            "065",
            f"[Çalışma Profili] -> UYGULANDI | Model: {self.model_key}",
        )
                ...
                if line_str:
                    ...
                    if is_stdout and log_stdout:
                        log_event(
                            PREFIX_TRL,
                            "068",
                            f"[Sistem Süreci] -> ÇIKTI (STDOUT) | {line_str}",
                        )
            stream.close()

        t_out = threading.Thread(
            target=_read_stream,
            args=(self.active_proc.stdout, stdout_deque, True),
            daemon=True,
        )
        t_err = threading.Thread(
            target=_read_stream,
            args=(self.active_proc.stderr, stderr_deque, False),
            daemon=True,
        )
        t_out.start()
        t_err.start()
        ...
        while self.active_proc.poll() is None:
            if self._cancel.is_set():
                ...
                log_event(
                    PREFIX_TRL,
                    "072",
                    f"[Sistem Süreci] -> DURDURULDU (İPTAL) | Komut: {' '.join(command)}",
                    level="warning",
                )
                raise RuntimeError("cancelled")
            ...
            if watch_path is not None and total_bytes > 0:
                ...
                if new_label != self.bytes_label and (now - last_progress_time) >= 1.0:
                    ...
                    if global_total > 0:
                        self.percent = 10 + int(
                            ((global_downloaded + current_bytes) / global_total) * 50
                        )
                    self._send_progress()
        ...
        if self.active_proc.returncode != 0:
            for line in list(stderr_deque):
                log_event(
                    PREFIX_TRL,
                    "067",
                    f"[Sistem Süreci] -> ÇIKTI (STDERR) | {line}",
                    level="error",
                )
            raise RuntimeError(
                f"{failure_message}: exit code {self.active_proc.returncode}"
            )
    ...
    def _packages_ready(self) -> bool:
        try:
            return (
                all(
                    importlib.util.find_spec(name) is not None for name in PACKAGE_NAMES
                )
    ...
    def _is_ready(self) -> bool:
        return all(
            (self.model_dir / name).exists() for name in tuple(self.spec["ready_files"])
        )
    ...
    def _fail(self, message: str) -> None:
        ...
        elapsed_ms = (
            int((time.monotonic() - self._install_started_at) * 1000)
        ...
        log_error(
            PREFIX_TRL,
            "069",
            f"[Model Kurulumu] -> KURULUM BAŞARISIZ | Model: {self.model_key} | Süre(ms): {elapsed_ms} | Hata: {message}",
            "Yerel model kurulumu tamamlanamadı.",
        )
        self._send(
            "offline_model_error",
            {"message": message, "model": self.model_key, "elapsed_ms": elapsed_ms},
        )
        self._send_status()
    ...
    def _log_dir_snapshot(self, directory: Path, label: str) -> None:
        if not directory.exists():
            log_event(
                PREFIX_TRL,
                "070",
                f"[Sistem Temizliği] -> KLASÖR BULUNAMADI | Hedef: {directory} | Etiket: {label}",
                level="warning",
            )
            return
        items: list[str] = []
        ...
        log_event(
            PREFIX_TRL,
            "071",
            f"[Sistem Temizliği] -> SİLİNDİ | İçerik: {', '.join(items) if items else 'boş'} | Etiket: {label}",
        )
