...
from core.translation.offline_model_manager import DEFAULT_MODEL_KEY, MODEL_SPECS, OfflineModelManager
...
class OfflineTranslationEngine(TranslationEngine):
    ...
    def __init__(self, cache: TranslationCache, models_dir: Path, bridge=None):
        ...
        self._translator_map: dict[str, object | None] = {key: None for key in MODEL_SPECS}
        self._tokenizer_map: dict[str, object | None] = {key: None for key in MODEL_SPECS}
        self._runtime_advice_map: dict[str, dict | None] = {key: None for key in MODEL_SPECS}
    ...
    def translate(self, text: str, src: str, tgt: str) -> tuple[str, str]:
        ...
        try:
            ...
            self.logger.error(f"[{PREFIX_TRL}-011] [Yerel Çeviri Motoru] -> ÇEVİRİ HATASI | Detay: {exc}")
            from core.errors import emit_bridge_event
            emit_bridge_event("log_entry", {
                "timestamp": "", "level": "ERROR", "prefix": "TRL", "code": "TRL-011",
            ...
            try:
                ...
                self.logger.error(f"[{PREFIX_TRL}-011] [Yerel Çeviri Motoru] -> YENİDEN BAŞLATMA BAŞARISIZ | Detay: {retry_exc}")
    ...
    def download_models(self, model_key: str | list[str] | tuple[str, ...] | None = None) -> None:
        requested = self._normalize_model_list(model_key)
        requested = [key for key in requested if not self._managers[key].get_status().get("available")]
        if not requested:
    ...
    def get_status(self) -> dict:
        statuses = {key: manager.get_status() for key, manager in self._managers.items()}
        busy_model = next((key for key, status in statuses.items() if status.get("busy")), None)
        ...
        active_action = "remove" if focus_state == "remove" and active_model else "install" if active_model else None
        return {
            "available": bool(selected.get("available", False)),
            "busy": bool(busy_model),
            "packages_ready": all(bool(status.get("packages_ready", False)) for status in statuses.values()),
            "models_ready": {key: bool(status.get("available", False)) for key, status in statuses.items()},
    ...
    def set_runtime_profile(self, performance_tier: str | None = None) -> None:
        ...
        profile_to_save = {"revision": OFFLINE_RUNTIME_PROFILE_REVISION, **self._runtime_profile}
        self._manager.save_runtime_profile(profile_to_save)
        self.logger.info(f"[{PREFIX_TRL}-018] Offline runtime profili uygulandi: {normalized} ({self._model_key})")
    ...
    def warmup(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"[{PREFIX_TRL}-012] [Yerel Çeviri Motoru] -> ISINMA (WARMUP) HATASI | Detay: {exc}")
            return False
    ...
    def _load_runtime(self) -> None:
        with self._load_lock:
            if self._translator_map[self._model_key] is not None and self._tokenizer_map[self._model_key] is not None:
                return
            ...
            if advice is None:
                try:
                    ...
                    self.logger.error(f"[{PREFIX_SYS}-019] [Donanım Taraması] -> TARAMA BAŞARISIZ | Aksiyon: İşlemciye (CPU) Geçiliyor | Hata: {exc}")
                    from core.errors import emit_bridge_event
                    emit_bridge_event("log_entry", {
                        "timestamp": "", "level": "WARNING", "prefix": "SYS", "code": "SYS-019",
                        "message": "Ekran kartına erişilemiyor. İşlemci ile devam ediliyor — performans düşebilir."
                    })
                    advice = {"device": "cpu", "compute_type": "int8", "inter_threads": 1, "intra_threads": 4, "reason": "fallback after scan error"}
            ...
            try:
                ...
                if advice["device"] != "cpu":
                    self.logger.warning(f"[{PREFIX_SYS}-020] CUDA yukleme basarisiz, CPU fallback: {exc}")
                    from core.errors import emit_bridge_event
                    emit_bridge_event("log_entry", {
                        "timestamp": "", "level": "WARNING", "prefix": "SYS", "code": "SYS-020",
                        "message": "Model ekran kartına yüklenemedi. İşlemci kullanılıyor."
                    })
                    self._runtime_advice_map[self._model_key] = {"device": "cpu", "compute_type": "int8", "inter_threads": 1, "intra_threads": 4, "reason": "cpu fallback"}
                    self.logger.info(f"[{PREFIX_TRL}-034] Offline translator CPU fallback deneniyor: model={self._model_key} model_dir={self.model_dir}")
                    self._translator_map[self._model_key] = ctranslate2.Translator(
                        str(self.model_dir),
                        device="cpu",
                        compute_type="int8",
            ...
            with warnings.catch_warnings(), redirect_stdout(StringIO()), redirect_stderr(StringIO()):
                ...
                try:
                    if self._model_key == "opus_mt_en_tr":
                        ...
                        self.logger.info(f"[{PREFIX_TRL}-035] MarianTokenizer yuklendi: {type(tokenizer).__name__}")
                    else:
                        warnings.filterwarnings(
                            "ignore",
                            message=r"The tokenizer you are loading from '.*' with an incorrect regex pattern:.*",
                        )
                        tokenizer = AutoTokenizer.from_pretrained(str(self.model_dir), src_lang=LANGUAGE_CODES["en"], use_fast=False)
                        self.logger.info(f"[{PREFIX_TRL}-035] AutoTokenizer yuklendi: {type(tokenizer).__name__}")
    ...
    def _run_opus_model(self, text: str) -> str:
        ...
        try:
            ...
            if "memory" in err_str or "cuda" in err_str or "cublas" in err_str or "alloc" in err_str:
                self.logger.warning(f"[{PREFIX_TRL}-099] VRAM doldu (OOM). Model CPU'ya dusuruluyor! Detay: {exc}")
                self.unload_runtime()
    ...
    def _resolve_source_language(self, text: str, src: str) -> str:
        ...
        return "ru" if cyrillic_count >= 2 and cyrillic_count >= max(2, latin_count) else "en"

    def _log_runtime_probe(self, label: str) -> None:
        try:
            files = sorted(p.name for p in self.model_dir.iterdir()) if self.model_dir.exists() else []
            self.logger.info(f"[{PREFIX_TRL}-036] Runtime probe {label}: model={self._model_key} model_dir={self.model_dir} files={files}")
            for name in ("config.json", "tokenizer_config.json"):
                path = self.model_dir / name
                if not path.exists():
                    self.logger.warning(f"[{PREFIX_TRL}-037] Runtime probe {label}: eksik dosya {name}")
                    continue
                payload = json.loads(path.read_text(encoding="utf-8"))
                null_keys = sorted(key for key, value in payload.items() if value is None)
                self.logger.info(
            ...
            self.logger.warning(f"[{PREFIX_TRL}-039] Runtime probe {label} okunamadi: {exc}")
    ...
    def _load_saved_runtime_profile(self) -> None:
        ...
        if int(saved.get("revision", 0) or 0) == OFFLINE_RUNTIME_PROFILE_REVISION:
            for key in ("beam_size", "repetition_penalty", "no_repeat_ngram_size", "length_ratio", "max_decoding_cap"):
                if key in saved:
    ...
    def _normalize_model_list(self, model_key: str | list[str] | tuple[str, ...] | None) -> list[str]:
    ...
    def send(self, event: str, data: dict) -> None:
        ...
        if event == "offline_model_complete":
            completed_model = str(data.get("model") or self._active_install_model or "").strip().lower()
            self._active_install_model = None
            if self._install_queue:
                next_model = self._install_queue.pop(0)
                self._active_install_model = next_model
                completed_label = MODEL_SPECS.get(completed_model, MODEL_SPECS[DEFAULT_MODEL_KEY])["label"]
        ...
        if event in {"offline_model_cancelled", "offline_model_error"}:
            ...
            self._active_install_model = None
        self.bridge.send(event, data)
        if event in {"offline_model_complete", "offline_model_cancelled", "offline_model_error"}:
            self.bridge.send("offline_model_status", self.get_status())
