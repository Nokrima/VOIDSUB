...
class ImageProcessor:
    ...
    def detect_scene_mode(self, image: np.ndarray) -> tuple[str, dict[str, float]]:
        try:
            ...
            low_sat_ratio = float(np.mean(bottom_hsv[:, :, 1] < 38)) if bottom_hsv.size else 0.0
            translucent_band_hint = float(np.mean((bottom_hsv[:, :, 1] < 46) & (bottom > 30) & (bottom < 140))) if bottom_hsv.size else 0.0
            ...
            floating_score = min(1.0, min(text_ratio * 14.0, 0.6) + min(contrast / 58.0, 0.4))
    ...
    def _build_text_mask(self, image: np.ndarray) -> np.ndarray:
        hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
        white = cv2.inRange(hsv, np.array([0, 0, self.white_v_min]), np.array([180, 65, 255]))
        yellow = cv2.inRange(hsv, np.array([15, 60, 130]), np.array([45, 255, 255]))
        mask = cv2.bitwise_or(white, yellow)
        return cv2.dilate(mask, np.ones((3, 3), np.uint8), iterations=1)

    def _process_striped(self, enhanced: np.ndarray, text_mask: np.ndarray) -> np.ndarray:
    ...
    def _process_floating(self, enhanced: np.ndarray, text_mask: np.ndarray) -> np.ndarray:
    ...
    def _focus_text_region(self, image: np.ndarray) -> np.ndarray:
        try:
            ...
            white_mask = cv2.inRange(hsv, np.array([0, 0, 120]), np.array([180, 70, 255]))
            yellow_mask = cv2.inRange(hsv, np.array([12, 50, 120]), np.array([48, 255, 255]))
            ...
            merged = cv2.morphologyEx(merged, cv2.MORPH_CLOSE, np.ones((3, 9), np.uint8), iterations=2)
    ...
    def process(self, image: np.ndarray, scene_mode: str = "striped") -> tuple[np.ndarray, int]:
        try:
            height, width = image.shape[:2]
            ...
            # Geniş ama kısa (altyazı bandı gibi) alanları veya genel olarak küçük alanları OCR netliği için büyüt
            if height <= 160 or (width <= 600 and height <= 360):
                img_resized = cv2.resize(image, (width * 2, height * 2), interpolation=cv2.INTER_CUBIC)
            else:
                max_w = 1280
                if width > max_w:
                    scale = max_w / width
            ...
            clip_limit = self.clahe_clip_floating if scene_mode == "floating" else self.clahe_clip_striped
            ...
            if scene_mode == "striped":
                ...
                processed = cv2.morphologyEx(processed, cv2.MORPH_CLOSE, np.ones((2, 2), np.uint8))
    ...
    def process_variants(self, image: np.ndarray, scene_mode: str = "striped", filters_enabled: bool = True) -> tuple[str, dict[str, float], list[tuple[str, np.ndarray, int]]]:
        variants: list[tuple[str, np.ndarray, int]] = []
        detected_mode, scene_scores = self.detect_scene_mode(image)
        preferred_mode = scene_mode if scene_mode in {"striped", "floating"} else detected_mode
        raw_prepared = self.prepare_raw(image)
        raw_focused = self._focus_text_region(image) if preferred_mode == "floating" else raw_prepared

        if not filters_enabled:
            return detected_mode, scene_scores, [
                (f"{preferred_mode}:raw-bgr", raw_prepared, self._background_intensity(raw_prepared)),
            ]

        if preferred_mode == "floating":
            variants.append((f"{preferred_mode}:raw-bgr", raw_prepared, self._background_intensity(raw_prepared)))
            if raw_focused.shape[:2] != raw_prepared.shape[:2] or not np.array_equal(raw_focused, raw_prepared):
                variants.append((f"{preferred_mode}:raw-focus", raw_focused, self._background_intensity(raw_focused)))

            try:
                gray_focus = cv2.cvtColor(raw_focused, cv2.COLOR_BGR2GRAY)
                clahe = cv2.createCLAHE(clipLimit=max(1.0, self.clahe_clip_floating), tileGridSize=(8, 8))
                gentle = clahe.apply(gray_focus)
                variants.append((f"{preferred_mode}:raw-contrast", gentle, int(np.mean(gentle) > 127)))
            except Exception as exc:
        ...
        try:
            for index, mode in enumerate(mode_order):
                ...
                if getattr(processed, "ndim", 0) == 2:
                    stroke = cv2.morphologyEx(processed, cv2.MORPH_GRADIENT, np.ones((2, 2), np.uint8))
                    variants.append((f"{mode}:stroke", stroke, int(np.mean(stroke) > 127)))
                    tophat = cv2.morphologyEx(processed, cv2.MORPH_TOPHAT, np.ones((3, 3), np.uint8))
                    variants.append((f"{mode}:tophat", tophat, int(np.mean(tophat) > 127)))
                    blackhat = cv2.morphologyEx(processed, cv2.MORPH_BLACKHAT, np.ones((3, 3), np.uint8))
                    variants.append((f"{mode}:blackhat", blackhat, int(np.mean(blackhat) > 127)))
                    if mode == "striped":
                        inv = cv2.bitwise_not(processed)
                        variants.append((f"{mode}:inverse", inv, int(np.mean(inv) > 127)))
                        band_emphasis = cv2.GaussianBlur(processed, (0, 0), 0.8)
                        band_emphasis = cv2.addWeighted(processed, 1.45, band_emphasis, -0.45, 0)
                        variants.append((f"{mode}:band", band_emphasis, int(np.mean(band_emphasis) > 127)))
                    if mode == "floating":
                        edge = cv2.Canny(processed, 40, 120)
                        variants.append((f"{mode}:edge", edge, int(np.mean(edge) > 127)))
        except Exception as exc:
    ...
    def find_dialog_bubbles(self, image: np.ndarray, min_area: int = 4000) -> list[tuple[int, int, int, int]]:
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
