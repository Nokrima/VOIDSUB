...
from core.processor.utils import _quick_normalize, _strip_speaker
...
class TranslationPipeline:
    def __init__(self, bridge, capturer):
        ...
        self.offline_translator = OfflineTranslationEngine(
            cache=self.tr_cache, models_dir=MODELS_DIR, bridge=bridge
        )
        ...
        self.loop_interval = max(
            0.04, float(self._profile_value("target_ms", 300)) / 1000
        )
        ...
        self._pending_translations: deque[tuple[str, int, float, float, float, str]] = (
            deque(maxlen=3)
        )
        ...
        self.source_state = SourceStateMachine(
    ...
    def _log_debug(
        self, prefix: str, code: str, message: str, correlation_id: str = ""
    ) -> None:
    ...
    async def _startup_phase(self) -> bool:
        ...
        self.bridge.send(
            "translation_state",
            {"running": False, "loading": True, "engine": runtime_engine},
        )
        await asyncio.sleep(0.05)
        started = await asyncio.get_running_loop().run_in_executor(
            None, self._activate_engine, runtime_engine
        )
        if not started:
            self.logger.error(
                f"[{PREFIX_SYS}-045] [OCR Motoru] -> BAŞLATILAMADI | Motor: {runtime_engine}"
            )
            engine_obj = self._get_engine_instance(runtime_engine)
            start_error = (
                getattr(engine_obj, "start_error", None) if engine_obj else None
            )
            self.bridge.send(
                "translation_state",
                {
        ...
        if getattr(self.capturer, "_capture_state", "ready") == "unavailable":
            ...
            self.logger.error(
                f"[{PREFIX_SYS}-045] [Ekran Yakalama] -> MODÜL BOZUK | Detay: {capture_err}"
            )
            self.bridge.send(
                "translation_state",
                {
        ...
        self.bridge.send(
            "translation_state", {"running": True, "engine": runtime_engine}
        )
        self.logger.info(
            f"[{PREFIX_SYS}-040] Ceviri hatti baslatildi. Motor: {runtime_engine}"
        )
        if self.translation_engine == "offline" and hasattr(
            self.offline_translator, "warmup"
        ):
            await asyncio.get_running_loop().run_in_executor(
                None, self.offline_translator.warmup
            )
    ...
    async def start_loop(self):
        if self.is_running:
            log_event(
                PREFIX_SYS,
                "075",
                "Start loop request ignored: pipeline already running",
                level="debug",
            )
            return
        ...
        try:
            while self.is_running:
                try:
                    ...
                    self.logger.error(
                        f"[{PREFIX_SYS}-041] [Ana Çeviri Döngüsü İçi] -> AKSAMA | Hata: {loop_exc}"
                    )
            ...
            self.logger.error(
                f"[{PREFIX_SYS}-041] [Ana Çeviri Döngüsü] -> KESİNTİ/AKSAMA | Hata: {exc}"
            )
            await asyncio.sleep(max(1.0, self.loop_interval))
        finally:
            if self._capture_task is not None:
                ...
                try:
                    ...
                    self.logger.error(
                        f"[{PREFIX_SYS}-048] [Ekran Yakalama] -> KAPATMA HATASI | Hata: {exc}"
                    )
    ...
    async def _run_loop_tick(self) -> None:
        ...
        if self.ocr_engine is None or not self.ocr_engine.is_ready():
            ...
            self.bridge.send(
                "translation_state",
                {"running": True, "loading": True, "engine": runtime_engine},
            )
            await asyncio.sleep(0.05)
            started = await asyncio.get_running_loop().run_in_executor(
                None, self._activate_engine, runtime_engine
        ...
        frame_started_monotonic = float(
            (capture_probe or {}).get("captured_monotonic", time.monotonic())
        )
        ...
        if frame_hash == self._last_frame_hash and self.last_detected_text:
            reused_done = await self._handle_reused_frame(
                frame_id, frame, frame_started_monotonic
            )
            if reused_done:
                return
    ...
    async def _handle_reused_frame(
        self, frame_id: int, frame, frame_started_monotonic: float
    ) -> bool:
        ...
        min_slot_samples = self._required_slot_samples(
            self.last_detected_text, self.last_detected_quality
        )
        ...
        if stabilized_text:
            self.overlay_publisher._emit_translation(
                stabilized_text,
                frame_started_monotonic=frame_started_monotonic,
                ocr_duration_ms=0.0,
            )
        del frame
        await asyncio.sleep(min(self.loop_interval, 0.01))
    ...
    async def _run_ocr_with_timeout(
        self, frame, frame_id: int, resolved_region
    ) -> tuple:
        ...
        try:
            ...
            self.logger.error(
                f"[{PREFIX_OCR}-051] [OCR İşlemi] -> ZAMAN AŞIMI | Çerçeve ID: {frame_id}"
            )
            ocr_payload = None
        ocr_duration_ms = (time.perf_counter() - ocr_started) * 1000
        return ocr_payload, ocr_duration_ms

    def _extract_detected_text(
        self, frame_id: int, ocr_payload: dict, ocr_duration_ms: float
    ) -> tuple | None:
        ...
        clean_report = (
            clean_ocr_source_detailed(raw_detected_text)
            if not self.raw_translation_flow_enabled
            else None
        )
        detected_text = (
            raw_detected_text
            if self.raw_translation_flow_enabled or not clean_report
            else str(clean_report["text"])
        )
        if self.raw_translation_flow_enabled:
            self.overlay_publisher._log_ocr(
                "033",
                f"Raw flow cleanup bypass: frame_id={frame_id}, raw_text={raw_detected_text!r}, cleaned_candidate=None",
            )
            self.overlay_publisher._log_ocr(
                "013",
        ...
        if (
            not self.raw_translation_flow_enabled
            and len(detected_text.strip()) < self._effective_min_text_chars()
        ):
            self.overlay_publisher._log_ocr(
                "017",
                f"Min text chars gate: decision=REJECTED, frame_id={frame_id}, length={len(detected_text.strip())}, threshold={self._effective_min_text_chars()}, text={detected_text!r}",
            )
            self.overlay_publisher._emit_frame_stat(
                ocr_payload, "rejected", "min_text_chars"
            )
            return None
        return detected_text, quality_score

    def _run_junk_filter(
        self,
        frame_id: int,
        frame,
        ocr_payload: dict,
        ...
        if self.raw_translation_flow_enabled:
            ...
            self.overlay_publisher._log_ocr(
                "014", f"Tip2 analysis: frame_id={frame_id}, decision=BYPASSED"
            )
            return False
    ...
    def _run_quality_gate(
        ...
        if not self.raw_translation_flow_enabled:
            quality_threshold, quality_reason = self._quality_gate_context(
                detected_text, quality_score
            )
            quality_rejected = self._should_drop_for_quality(
                detected_text, quality_score
            )
        if self.raw_translation_flow_enabled:
            self.overlay_publisher._log_ocr(
                "034",
                f"Raw flow gates bypassed: frame_id={frame_id}, min_chars=off, junk=off, quality=off, score={quality_score}, threshold={quality_threshold}",
            )
        self.overlay_publisher._log_ocr(
            "019",
        ...
        if quality_rejected:
            ...
            log_event(
                PREFIX_SYS,
                "033",
                "[Görüntü İşleme] -> ATLANDI | Düşük kalite OCR çıktısı",
                throttle_key="quality_skip",
                throttle_seconds=2.0,
            )
            self.overlay_publisher._emit_frame_stat(ocr_payload, "rejected", "quality")
        return quality_rejected

    def _apply_text_gates(
        self, frame_id: int, frame, ocr_payload: dict, ocr_duration_ms: float
    ):
        ...
        if self._run_junk_filter(
            frame_id, frame, ocr_payload, detected_text, quality_score
        ):
            return None
        if self._run_quality_gate(
            frame_id, frame, ocr_payload, detected_text, quality_score
        ):
            return None
        return detected_text, quality_score
    ...
    def _run_stabilizer(
        ...
        if push_result in ("new_slot", "rejected"):
            ...
            if self._instability_count > 5:
                ...
                self.overlay_publisher._log_ocr(
                    "060",
                    "Ekran içeriği çok hızlı değişiyor (Flickering tespit edildi).",
                )
                emit_bridge_event(
                    "log_entry",
                    {
        ...
        if push_result == "rejected":
            self.overlay_publisher._emit_frame_stat(
                ocr_payload, "rejected", "slot_rejected"
            )
            del frame
            del ocr_payload
            return False
        if self.slot_manager.get_sample_count() < min_slot_samples:
            self.overlay_publisher._log_ocr(
                "021",
                f"Stabilizer decision: ACCEPTED=NO, frame_id={frame_id}, samples={self.slot_manager.get_sample_count()}, required={min_slot_samples}, reason=slot_wait",
            )
            self.overlay_publisher._emit_frame_stat(
                ocr_payload, "rejected", "slot_wait"
            )
            del frame
            del ocr_payload
            return False
        ...
        self.overlay_publisher._log_ocr(
            "023",
            f"Stabilizer decision: ACCEPTED=YES, frame_id={frame_id}, samples={self.slot_manager.get_sample_count()}, required={min_slot_samples}, final_text={stabilized_text!r}",
        )
        self.diagnostics.record(
            "accepted",
            self.active_engine,
        ...
        self.overlay_publisher._log_ocr(
            "024",
            f"Final text queued for translation: frame_id={frame_id}, text={stabilized_text!r}, ocr_ms={ocr_duration_ms:.1f}",
        )
        self.overlay_publisher._emit_translation(
            stabilized_text,
            frame_started_monotonic=frame_started_monotonic,
    ...
    async def _process_ocr_frame(
        ...
        ocr_payload, ocr_duration_ms = await self._run_ocr_with_timeout(
            frame, frame_id, resolved_region
        )
        if not ocr_payload:
            del frame
            return False
        gate_result = self._apply_text_gates(
            frame_id, frame, ocr_payload, ocr_duration_ms
        )
        ...
        if self.raw_translation_flow_enabled:
            ...
            self.overlay_publisher._log_ocr(
                "024",
                f"Raw flow queued for translation: frame_id={frame_id}, text={detected_text!r}, ocr_ms={ocr_duration_ms:.1f}",
            )
            self.overlay_publisher._emit_translation(
                detected_text,
                frame_started_monotonic=frame_started_monotonic,
        ...
        while self.is_running:
            try:
                ...
                if overlay is not None:
                    overlay_hidden_for_capture = await asyncio.to_thread(
                        overlay.prepare_capture, region_snapshot
                    )
                try:
                    frame = await asyncio.to_thread(
                        self.capturer.capture_region, region_snapshot
                    )
                finally:
                    if overlay is not None:
                        await asyncio.to_thread(
                            overlay.finish_capture, overlay_hidden_for_capture
                        )
                resolved_region = getattr(
                    self.capturer, "get_last_resolved_region", lambda: None
                )()
                if frame is not None:
                    capture_ms = (time.perf_counter() - capture_started) * 1000
                    self._latest_frame = frame
                    self._latest_frame_id += 1
                    self._latest_resolved_region = (
                        dict(resolved_region)
                        if isinstance(resolved_region, dict)
                        else None
                    )
                    self._latest_capture_probe = {
                        ...
                        "resolved_region": dict(resolved_region)
                        if isinstance(resolved_region, dict)
                        else None,
                ...
                self.logger.error(
                    f"[{PREFIX_SYS}-047] [Ekran Yakalama Döngüsü] -> ÜRETİCİ AKSADI | Hata: {exc}"
                )
    ...
    def update_config(
        ...
        if engine_id and engine_id in ["winonly", "easy"]:
            ...
            self.source_state.configure(
                hold_window_ms=int(self._profile_value("source_family_hold_ms", 1600))
            )
            self.logger.info(
                f"[{PREFIX_SYS}-043] Motor degisimi hatta uygulandi: {engine_id}"
            )
        if region:
        ...
        if translation_engine:
            ...
            log_event(
                PREFIX_CFG,
                "012",
                f"[Çeviri Ayarları] -> UYGULANDI | Motor: {translation_engine}",
                throttle_key="translation_engine_cfg",
                throttle_seconds=0.2,
            )
            tier = get_performance_tier_profile(
                self.active_engine, self.performance_tier, self.translation_engine
            )
            self.loop_interval = max(0.04, float(tier["target_ms"]) / 1000)
            self.source_state.configure(
                hold_window_ms=int(tier.get("source_family_hold_ms", 1600))
            )
            self.overlay_publisher._log_translation_policy(tier)
        if offline_model_key is not None and hasattr(
            self.offline_translator, "set_model_key"
        ):
            normalized_model_key = (
                str(offline_model_key or "opus_mt_en_tr").strip().lower()
            )
        ...
        if performance_tier:
            ...
            tier = get_performance_tier_profile(
                self.active_engine, performance_tier, self.translation_engine
            )
            self.loop_interval = max(0.04, float(tier["target_ms"]) / 1000)
            self.source_state.configure(
                hold_window_ms=int(tier.get("source_family_hold_ms", 1600))
            ...
            log_event(
                PREFIX_CFG,
                "013",
                f"[Performans Profili] -> UYGULANDI | Seviye: {performance_tier} | Motor: {self.active_engine} | Hedef(ms): {tier['target_ms']}",
                throttle_key="performance_tier_cfg",
                throttle_seconds=0.2,
            )
            self.overlay_publisher._log_translation_policy(tier)
    ...
    def _activate_engine(self, engine_id: str) -> bool:
        ...
        if engine is None:
            log_event(
                PREFIX_OCR,
                "051",
                f"[OCR Motoru Aktivasyonu] -> AKTİVASYON BAŞARISIZ | Bilinmeyen Motor: {engine_id}",
                level="error",
            )
            return False
        log_event(
        ...
        if self.ocr_engine is not engine:
            if self.ocr_engine is not None:
                log_event(
                    PREFIX_OCR,
                    "053",
                    f"Stopping previous OCR engine: class={type(self.ocr_engine).__name__}",
                    level="debug",
                )
                self.ocr_engine.stop()
        ...
        if self.ocr_engine is None:
            log_event(
                PREFIX_OCR,
                "051",
                f"[OCR Motoru Aktivasyonu] -> AKTİVASYON BAŞARISIZ | Motor Referansı Yok: {engine_id}",
                level="error",
            )
            return False
        if self.ocr_engine.is_ready():
            log_event(
                PREFIX_OCR,
                "054",
                f"[OCR Motoru Aktivasyonu] -> ATLANDI | Zaten Hazır: {engine_id}",
            )
            return True
    ...
    def _take_latest_frame(
        self,
    ) -> tuple[int, np.ndarray, dict | None, dict | None] | None:
        ...
        return (
            ...
            dict(self._latest_resolved_region)
            if isinstance(self._latest_resolved_region, dict)
            else None,
            dict(self._latest_capture_probe)
            if isinstance(self._latest_capture_probe, dict)
            else None,
        )
    ...
    def _read_fast_then_refine(self, frame: np.ndarray, frame_id: int) -> dict | None:
        if self.ocr_engine is None:
            log_event(
                PREFIX_OCR,
                "057",
                f"[OCR İşlemi] -> ATLANDI | Neden: Motor Yok | Çerçeve ID: {frame_id}",
                level="warning",
            )
            return None

        detected_scene_mode, scene_scores, variants = (
            self.image_processor.process_variants(
                frame, self.ocr_scene_mode, self.ocr_filters_enabled
            )
        )
        self.overlay_publisher._log_ocr(
            "004",
        ...
        for index in range(fast_pass_budget):
            payload = self._read_variant(
                frame, frame_id, variants[index], detected_scene_mode, scene_scores
            )
    ...
    def _should_skip_refine(self, frame_id: int, payload: dict | None) -> bool:
        ...
        if self._latest_frame_id != frame_id and self._is_subtitle_active():
            return True
        if (
            self._is_subtitle_active()
            and quality_score >= max(self.quality_threshold - 8, 32)
            and scene_fit >= self.scene_fit_threshold
        ):
            return True
        return quality_score >= max(
            self.quality_threshold - 2, 38
        ) and scene_fit >= min(0.9, self.scene_fit_threshold + 0.10)
        ...
        if self._is_subtitle_active():
            return (
                ...
                >= max(
                    ...
                    - int(self._profile_value("active_quality_relax", 8)),
                    int(self._profile_value("fast_quality_floor", 30)),
                )
                and scene_fit >= self.scene_fit_threshold
                and text_length <= int(self._profile_value("fast_text_len", 12)) + 6
            )
        return quality_score >= max(
            self.quality_threshold + 8, 58
        ) and scene_fit >= min(0.85, self.scene_fit_threshold + 0.08)
    ...
    def _evaluate_tip2_best_variant_gate(
        self, analysis: TextAnalysisResult
    ) -> dict[str, Any]:
        ...
        if (
            unknown_long >= 1
            and recognized_ratio < 0.40
            and (broken >= 1 or merged_hits)
        ):
    ...
    def _should_skip_family_repeat(self, stabilized_text: str) -> bool:
        ...
        hold_window_ms = int(self._profile_value("source_family_hold_ms", 1600))
        within_window = (
            time.monotonic() - self._last_emitted_source_time
        ) * 1000 < hold_window_ms
        ...
        last_normalized = re.sub(
            r"\s+", " ", _strip_speaker(self._last_emitted_source_text).lower()
        ).strip()
        ...
        if (
            ...
            and (
                current_mojibake > last_mojibake or current_suspicious > last_suspicious
            )
        ):
            self.overlay_publisher._log_ocr(
                "025",
    ...
    def _is_meaningful_source_upgrade(
        self, current_normalized: str, last_normalized: str
    ) -> bool:
        if not current_normalized or not last_normalized:
            return False
        if current_normalized == last_normalized:
    ...
    def _stabilize_text(
        self, detected_text: str, quality_score: int, repeated_frame: bool = False
    ) -> str | None:
        ...
        if (
            line_count == 1
            and active_subtitle_mode
            and repeated_frame
            and text_length <= fast_text_len
            and quality_score
            >= max(self.quality_threshold - active_quality_relax, fast_quality_floor)
        ):
        ...
        if (
            ...
            and repeated_frame
            and sentence_like
            and quality_score >= max(self.quality_threshold - 4, 38)
        ):
            return self.stabilizer.push(cleaned, min_samples=1, force=True)
        if (
            line_count == 1
            ...
            and repeated_frame
            and sentence_like
            and quality_score >= max(self.quality_threshold - 8, 30)
        ):
            return self.stabilizer.push(cleaned, min_samples=1, force=True)
        ...
        if text_length <= 12:
            min_samples = (
                1
                if repeated_frame
                and quality_score >= max(self.quality_threshold - 14, 22)
                else base_min_samples
            )
            if line_count >= 2:
                min_samples = max(min_samples, 3 if line_count == 2 else 4)
            return self.stabilizer.push(cleaned, min_samples=min_samples)
    ...
    def _quality_gate_context(self, text: str, quality_score: int) -> tuple[int, str]:
        ...
        if self._is_subtitle_active():
            threshold = max(
                18, threshold - int(self._profile_value("active_quality_relax", 8))
            )
        if length <= 4:
    ...
    def _mark_subtitle_activity(self, duration: float = 0.9) -> None:
        self._subtitle_active_until = max(
            self._subtitle_active_until, time.monotonic() + duration
        )
    ...
    def _capture_delay(self) -> float:
        base_delay = (
            min(
                self.loop_interval,
                float(self._profile_value("active_target_ms", 60)) / 1000,
            )
            if self._is_subtitle_active()
            else self.loop_interval
        )
    ...
    def _profile_value(self, key: str, default: Any) -> Any:
        tier = get_performance_tier_profile(
            self._runtime_engine_id(), self.performance_tier, self.translation_engine
        )
    ...
    def _should_skip_translated_emit(
        self, translated_text: str, source: str = ""
    ) -> bool:
        ...
        if self.raw_translation_flow_enabled:
            ...
            return False
        within_window = (
            time.monotonic() - self._last_translated_emit_time
        ) * 1000 < repeat_window_ms
        if not within_window:
            return False
    ...
    def _should_skip_raw_source_repeat(self, text: str) -> bool:
        ...
        now = time.monotonic()
        if (
            self._last_raw_source_text == normalized
            and (now - self._last_raw_source_time) * 1000 < 900
        ):
            return True
        if self._pending_translations:
            pending_normalized = self._normalize_translated_text(
                self._pending_translations[-1][0]
            )
            if pending_normalized == normalized:
                return True
        if (
            self._active_translation_source
            and self._normalize_translated_text(self._active_translation_source)
            == normalized
        ):
            return True
        return False
    ...
    def _normalize_ocr_text(self, text: str) -> str:
        ...
        cleaned = re.sub(
            r"([^\w\s.,!?'\-:;/%])\1{1,}", r"\1", cleaned, flags=re.UNICODE
        )
        ...
        cleaned = re.sub(
            r"\b([Tt]hat|[Ii]t|[Ww]hat|[Ww]ho|[Tt]here|[Hh]ere)'\$", r"\1's", cleaned
        )
    ...
    def _score_ocr_candidate(self, result: tuple, text: str) -> float:
        ...
        length = len(text)
        length_score = (
            1.0 if 8 <= length <= 140 else 0.9 if 4 <= length <= 220 else 0.58
        )
    ...
    def _extract_candidate_position(
        self, text: str, ocr_results: list[tuple]
    ) -> tuple[float, float]:
        for result in ocr_results:
            if (
                len(result) < 2
                or self._normalize_ocr_text(cast(str, result[1])) != text
            ):
                continue
            bbox = result[0]
            if isinstance(bbox, tuple) and len(bbox) == 4:
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"[{PREFIX_SYS}-099] [Sistem Kapatma] -> OFFLINE TEMİZLİK HATASI | Detay: {exc}"
            )
