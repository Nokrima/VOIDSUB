...
class TextQualityScorer:
    ...
    def score(cls, text: str) -> int:
        ...
        punctuation_ratio = sum(not char.isalnum() and not char.isspace() for char in cleaned) / length
        ...
        short_token_penalty = 12 if len(tokens) >= 3 and sum(len(token) <= 2 for token in tokens) >= max(2, len(tokens) // 2) else 0
        ...
        if length <= 12 and alpha_count >= 2 and digit_ratio < 0.25 and punctuation_ratio < 0.35:
