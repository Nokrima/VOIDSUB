...
class OverlayWidget(QWidget):
    ...
    def _apply_clickthrough(self, enabled: bool):
        ...
        ex_style |= (
            win32con.WS_EX_LAYERED | win32con.WS_EX_NOACTIVATE | win32con.WS_EX_TOPMOST
        )
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:  # type: ignore
            ...
            if self.controller.signals:
                self.controller.signals.drag_started.emit(
    ...
    def mouseMoveEvent(self, event):
        if event.buttons() & Qt.LeftButton and not getattr(
            self, "_drag_ignored", False
        ):  # type: ignore
            if self.controller.signals:
                self.controller.signals.drag_moved.emit(
                    event.globalPosition().x(), event.globalPosition().y()
                )
...
class ModernOverlay:
    ...
    def _bootstrap(self) -> None:
        try:
            ...
            self._ready.set()
            print(
                f"[OVERLAY DEBUG] CRASH inside _run_qt: {exc}\n{traceback.format_exc()}"
            )
            log_error(
                PREFIX_SYS,
                "018",
                f"[Arayüz (Overlay)] -> BAŞLATILAMADI | Hata: {exc}",
                "Modern overlay baslatilamadi.",
            )
    ...
    def _exact_text_height(self, text: str, width: int, size: int, weight: str) -> int:
        ...
        rect = metrics.boundingRect(
            0, 0, width, 10000, Qt.AlignLeft | Qt.AlignTop | Qt.TextWordWrap, text
        )  # type: ignore
        return rect.height()
    ...
    def _measure_window_width(self, texts: list[str], weight: str) -> int:
        ...
        if self.mode == "fixed":
            max_width = min(
                max(self._width, int(bounds["width"] * 0.9)), bounds["width"] - 32
            )
        else:
        ...
        for text in texts:
            size = (
                max(10, self.size - 2)
                if self.mode == "jump" and text != texts[-1]
                else self.size
            )
            font = QFont(self.font, size)
            font.setBold(weight == "bold")
            ...
            measured = (
                ...
                if "\n" not in text
                else max(metrics.horizontalAdvance(line) for line in text.splitlines())
            )
    ...
    def _place_near_region(self) -> None:
        ...
        safe_margin_x = 28
        target_width = min(
            self._width, max(220, min(self.w, bounds["width"] - (safe_margin_x * 2)))
        )
        ...
        if self.snap_to_region and self._last_region and not self._manual_position:
            ...
            max_left_span = max(
                220, (region_center_x - (bounds["left"] + safe_margin_x)) * 2
            )
            max_right_span = max(
                220,
                ((bounds["left"] + bounds["width"] - safe_margin_x) - region_center_x)
                * 2,
            )
            local_safe_width = max(
                220,
                min(
                    bounds["width"] - (safe_margin_x * 2), max_left_span, max_right_span
                ),
            )
            ...
            below_space = (
                bounds["top"] + bounds["height"] - (region_top + region_height) - 24
            )
    ...
    def _render(self) -> None:
        ...
        while current_size >= 10:
            total_content_height = 0
            if self.mode == "fixed":
                text_height = self._exact_text_height(
                    latest, card_width - 20, current_size, weight
                )
                total_content_height = text_height + 24
            elif self.mode == "jump":
                front_text_h = self._exact_text_height(
                    latest, card_width - 20, current_size, weight
                )
                total_content_height = front_text_h + 24
                if len(data) > 1:
                    back_width = int(card_width * 0.88)
                    back_text_h = self._exact_text_height(
                        data[-2], back_width - 20, max(10, current_size - 2), weight
                    )
                ...
                for i, text in enumerate(data):
                    ...
                    local_width = (
                        int(card_width * (0.88 + (i / max(1, len(data) - 1)) * 0.12))
                        if len(data) > 1
                        else card_width
                    )
                    total_content_height += (
                        self._exact_text_height(
                            text, local_width - 18, size_val, weight
                        )
                        + 16
                    )
    ...
    def _draw(self, painter: QPainter):
        ...
        def draw_card(left, top, right, bottom, color_str):
            ...
            path = QPainterPath()
            path.addRoundedRect(
                QRectF(left, top, right - left, bottom - top), radius, radius
            )
        ...
        def draw_text(x, y, txt, width, fill, size, is_bold):
                ...
                if blur_amt > 0:
                    ...
                    if len(txt) <= 200:
                        for bx, by in [
                            ...
                            painter.drawText(
                                b_rect, Qt.AlignCenter | Qt.TextWordWrap, txt
                            )  # type: ignore
                    painter.setOpacity(self._anim_progress)
            ...
            if self.shadow:
                ...
                rect_shadow = QRect(
                    int(x - width / 2), int(y - 1000 + 3), int(width), 2000
                )
        ...
        if self.mode == "fixed":
            ...
            draw_card(
                card_left, card_top, card_left + card_width, card_bottom, "#0A101A"
            )
            draw_text(
                cx,
        ...
        if self.mode == "jump":
            ...
            front_text_h = self._exact_text_height(
                latest, card_width - 20, self._computed_size, weight
            )
            ...
            if len(data) > 1:
                ...
                draw_text(
                    cx,
                    (back_top + back_bottom) // 2,
                    data[-2],
                    back_width - 20,
                    "#9CA3AF",
                    max(10, self._computed_size - 2),
                    self.bold,
                )
            else:
                draw_card(
    ...
    def set_style(self, font: str, size: int, color: str, bold: bool) -> None:
        if self.signals:
            self.signals.set_style.emit(font, size, color, bold)
    ...
    def set_region(self, region: dict | None) -> None:
        if self.signals:
            self.signals.set_region.emit(region)

    def update_snap_to_region(self, snap: bool) -> None:
        if self.signals:
            self.signals.set_snap_to_region.emit(snap)

    def apply_settings(self, settings: dict | None) -> None:
        if self.signals:
            self.signals.apply_settings.emit(settings or {})
    ...
    def toggle(self) -> None:
        if self.signals:
            self.signals.toggle_window.emit()

    def prepare_capture(self, region: dict | None) -> bool:
        if not isinstance(region, dict) or not self.window:
            return False
        ...
        if (
            not intersect(capture_rect, overlay_rect)
            or not self.visible
            or self._hidden_by_user
        ):
            return False
    ...
    def push_sequence(
        ...
        def calc_delay(txt):
            if (reading_speed or 60) <= 0:
                return 0
            return max(
                (min_display_ms or 1200),
                int((len(txt) / max(1, (reading_speed or 60))) * 1000) + 3000,
            )
        ...
        def _show_next(index):
            if not self.window or index >= len(ready_chunks):
                return
    ...
    def update_sequence(
        ...
        if (reading_speed or 60) <= 0:
            ...
            delay = max(
                (min_display_ms or 1200),
                int((len(chunk) / max(1, (reading_speed or 60))) * 1000) + 3000,
            )
    ...
    def _do_apply_settings(self, settings):
        ...
        self.mode = (
            requested_mode
            if requested_mode in {"waterfall", "jump", "fixed"}
            else "waterfall"
        )
        self.anim = str(settings.get("anim", self.anim))
        self.shadow = bool(settings.get("shadow", self.shadow))
        print(
            f"[OVERLAY DEBUG] Computed: font={self.font}, size={self.size}, color={self.color}, alpha={self.alpha}, bg={self.bg_visible}, mode={self.mode}, anim={self.anim}, shadow={self.shadow}"
        )
        self._render()
    ...
    def _do_display_changed(self):
        self.logger.info(
            f"[{PREFIX_SYS}-089] Ekran cozunurlugu veya duzeni degisti, overlay yeniden boyutlandiriliyor."
        )
        self._last_bounds = self._virtual_bounds()
        self._place_near_region()
        self._render()
