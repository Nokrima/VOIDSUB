...
class SettingsStore:
    ...
    def merge_payload(self, payload: dict) -> bool:
        ...
        if "custom_profiles" in app_data:
            app_data["custom_profiles"] = self._normalize_custom_profiles(
                app_data["custom_profiles"]
            )
    ...
    def _normalize_custom_profile(self, profile: Any) -> dict[str, Any] | None:
        ...
        return {
            ...
            "overlay_overrides": self._normalize_profile_overlay_overrides(
                profile.get("overlay_overrides", {})
            ),
            "app_overrides": self._normalize_profile_app_overrides(
                profile.get("app_overrides", {})
            ),
        }
    ...
    def _normalize_profile_overlay_overrides(self, overrides: Any) -> dict[str, Any]:
        ...
        if "font_size" in overrides:
            result["font_size"] = int(overrides["font_size"])
        if "font_weight" in overrides:
            result["font_weight"] = int(overrides["font_weight"])
        if "bg_opacity" in overrides:
            result["bg_opacity"] = float(overrides["bg_opacity"])
        if "text_color" in overrides:
            result["text_color"] = str(overrides["text_color"])
        if "bg_color" in overrides:
            result["bg_color"] = str(overrides["bg_color"])
        return result
    ...
    def _normalize_profile_app_overrides(self, overrides: Any) -> dict[str, Any]:
        if not isinstance(overrides, dict):
            return {}
        result = {}
        if "translation_engine" in overrides:
            result["translation_engine"] = str(overrides["translation_engine"])
        if "src_language" in overrides:
            result["src_language"] = str(overrides["src_language"])
        if "tgt_language" in overrides:
            result["tgt_language"] = str(overrides["tgt_language"])
        if "performance_preset" in overrides:
            result["performance_preset"] = str(overrides["performance_preset"])
        return result
    ...
    async def run(self, mode: str = "target") -> None:
        self.bridge.send(
            "log_entry",
            {
                ...
                "prefix": "UI",
                "code": "UI-100",
                "message": "Native region selector baslatiliyor.",
            },
        )
            ...
            if result.returncode == 0 and result.stdout.strip():
                try:
                    ...
                    if payload.get("cancelled"):
                        self.bridge.send(
                            "native_region_selection", {"status": "cancelled"}
                        )
                        return
                    ...
                    if normalized:
                        if mode == "calibration":
                            ...
                            if self.bridge.worker and getattr(
                                self.bridge.worker, "is_running", False
                            ):
                        ...
                        self.bridge.send(
                            "native_region_selection",
                            {"status": "failed", "error": "Invalid payload format"},
                        )
                except json.JSONDecodeError:
                    self.bridge.send(
                        "native_region_selection",
                        {"status": "failed", "error": "Invalid JSON from selector"},
                    )
            else:
                self.bridge.send(
                    "native_region_selection",
                    {
                        "status": "cancelled" if result.returncode == 1 else "failed",
        ...
        try:
            if event == "new_translation":
                ...
                if "overlay_snap_to_region" in app_set:
                    overlay.update_snap_to_region(
                        bool(app_set["overlay_snap_to_region"])
                    )
        except Exception as e:
            self.bridge.logger.error(
                f"[OverlayDispatcher] Failed to dispatch '{event}': {e}"
            )
...
class BridgeServer:
    def __init__(
        self, worker: Any = None, host: str = WEBSOCKET_HOST, port: int = WEBSOCKET_PORT
    ):
        self.worker = worker
        self.host = host
        self.port = port
        self.saved_regions: dict[str, dict[str, Any] | None] = {
            "calibration": None,
            "target": None,
        }
    ...
    def easyocr_manager(self):
        if not hasattr(self, "_easyocr_manager"):
            ...
            plugins_dir = (
                Path(os.environ.get("LOCALAPPDATA", "C:/")) / "VOIDSUB" / "plugins"
            )
    ...
    def attach_native_overlay(self, overlay: Any) -> None:
        ...
        self.overlay_dispatcher.dispatch(
            "app_settings", {"settings": self.store.app_settings}
        )
    ...
    def _restore_saved_regions(self) -> None:
        ...
        if "calibration_region" in settings:
            self.saved_regions["calibration"] = self._normalize_region(
                settings["calibration_region"]
            )
        if "target_region" in settings:
            self.saved_regions["target"] = self._normalize_region(
    ...
    def _emit_saved_regions(self) -> None:
        ...
        self.overlay_dispatcher.dispatch(
            "saved_regions_update", {"regions": self.saved_regions}
        )
    ...
    async def _run_engine_repair(self, engine_id: str) -> None:
        self.send(
            "log_entry",
            {
                ...
                "prefix": "SYS",
                "code": "SYS-055",
                "message": f"{engine_id} için onarım başlatılıyor...",
            },
        )
        try:
            if engine_id == "easy":
                ...
                if Path("easyocr_models").exists():
                    shutil.rmtree("easyocr_models", ignore_errors=True)
                EasyOCREngine()
                self.send("repair_result", {"engine": engine_id, "success": True})
            else:
                self.send(
                    "repair_result",
                    {"engine": engine_id, "success": False, "error": "Not supported"},
                )
        except Exception as e:
            self.send(
                "repair_result",
                {"engine": engine_id, "success": False, "error": str(e)},
            )
    ...
    async def handler(self, websocket):
        ...
        self.send(
            "hello",
            {
                "message": "VoidSub Core Bağlandı",
                "hw_info": self.hw_detector.scan_system(),
            },
        )
        self._emit_app_settings()
        self._emit_saved_regions()
        if self.temporary_region_active:
            self._emit_temporary_region_state()
            ...
            async for message in websocket:
                try:
                    ...
                    if event not in ("diagnostics_update"):
                        self.logger.debug(
                            f"[WS] <- {event}: {_clip_log_text(str(data))}"
                        )
                    ...
                    if event == "save_settings":
                            ...
                            if hasattr(self.worker, "update_config"):
                                ...
                                if "active_calibration_profile_id" in data:
                                    config_updates["calibration_profile_active"] = bool(
                                        data["active_calibration_profile_id"]
                                    )
                                if config_updates:
                                    try:
                                        ...
                                        self.logger.error(
                                            f"[BridgeServer] Worker update_config error: {e}"
                                        )
                        ...
                        if self.store.merge_payload({"overlay": data}):
                            ...
                            if self.native_overlay and hasattr(
                                self.native_overlay, "apply_settings"
                            ):
                        ...
                        event == "update_target_region" or event == "set_runtime_region"
                    ):
                        region_data = (
                            data.get("region")
                            if event == "set_runtime_region"
                            else data
                        )
                        ...
                        if self.native_overlay is not None:
                            ...
                            mode = str(
                                self.settings.get("overlay", {}).get("mode", "fixed")
                            )
                            speed = int(
                                self.settings.get("app", {}).get(
                                    "reading_speed_cps", 60
                                )
                            )
                            self.native_overlay.update_sequence(
                                [text], mode=mode, reading_speed=speed
                            )
                    elif event == "toggle_overlay_visibility":
                        if self.native_overlay is not None:
                            self.native_overlay.toggle()
                    elif event == "clear_overlay":
                        if self.native_overlay is not None and hasattr(
                            self.native_overlay, "clear"
                        ):
                            self.native_overlay.clear()
                    elif event == "repair_engine":
                        asyncio.create_task(
                            self._run_engine_repair(data.get("engine_id", "easy"))
                        )
                        ...
                        offline_engine = (
                            getattr(
                                self.worker,
                                "offline_engine",
                                getattr(self.worker, "offline_translator", None),
                            )
                            if self.worker
                            else None
                        )
                        if offline_engine is not None:
                            self.send(
                                "offline_model_status", offline_engine.get_status()
                            )
                        ...
                        self.send(
                            ...
                            {
                                "running": getattr(
                                    self.worker.pipeline, "is_running", False
                                )
                                if self.worker
                                else False,
                                "loading": getattr(
                                    self.worker, "_engine_loading", False
                                )
                                if self.worker
                                else False,
                                "active_engine": self.store.app_settings.get(
                                    "translation_engine", "easy"
                                ),
                                "recording": rec_state,
                            },
                        )
    ...
    async def start(self):
        ...
        server = await websockets.serve(self.handler, self.host, self.port)  # type: ignore
        ...
        self.logger.info(
            f"[BridgeServer] WebSocket basladi: ws://{self.host}:{self.port}"
        )
