...
def copy_sources():
    ...
    for src in SOURCES_TO_COPY:
        ...
        if src.is_dir():
            ...
            shutil.copytree(
                src, dest, ignore=shutil.ignore_patterns("__pycache__", "*.pyc", ".git")
            )
...
def compile_to_pyc():
    ...
    print_step(
        "Python dosyalari (.py) bytecode'a (.pyc) derleniyor ve asillari siliniyor..."
    )
    ...
    print(
        f"    Toplam {deleted_py_count} adet .py dosyasi .pyc formatina donusturuldu ve asillari silindi."
    )
...
def main():
    ...
    print("Gelistirilmis ve derlenmis Python ortaminiz surada hazir:")
