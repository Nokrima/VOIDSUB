...
class PreviewHandler:
        ...
        try:
            ...
            if isinstance(region, dict):
                ...
                if not pipeline:
                    ...
                    self.bridge.send("calibration_region_failed", {"message": "Çeviri motoru hazır değil."})
                    return
                frame = await asyncio.to_thread(pipeline.capturer.capture_region, region)
                if frame is None:
                    logger.error("[DBG-010] Failed to capture region frame.")
                    self.bridge.send("calibration_region_failed", {"message": "Ekran yakalanamadı. Lütfen modülleri kontrol edin (Örn: winrt paketi bozuk)."})
                    return
            ...
            if payload.get("cancelled"):
                self.bridge.send("calibration_region_cancelled", {
    ...
    async def _request_preview_async(self, payload: dict) -> None:
        ...
        if not pipeline:
            self.bridge.send("calibration_preview_result", {"error": "Pipeline hazir degil."})
            return

        def _process() -> dict:
            started_at = time.perf_counter()
            frame = self.recorder.preview_frame
            scene_mode = str(payload.get("scene_mode") or getattr(pipeline, "ocr_scene_mode", "striped"))
            if scene_mode not in {"striped", "floating"}:
                scene_mode = "striped"
            img_proc = ConfigurableImageProcessor(config)

            filters_enabled = bool(payload.get("ocr_filters_enabled", getattr(pipeline, "ocr_filters_enabled", True)))
            if filters_enabled:
                processed_preview, _ = img_proc.process(frame, scene_mode)
            else:
                processed_preview = img_proc.prepare_raw(frame)

            d_mode, sc_scores, variants = img_proc.process_variants(frame, scene_mode, filters_enabled)
            ...
            best_payload = None
            preview_budget = max(1, int(config.get("variant_budget", getattr(pipeline, "variant_budget_override", None) or pipeline._variant_budget())))
            for index, variant in enumerate(variants[:preview_budget], start=1):
                variant_payload = pipeline._read_variant(frame, index, variant, d_mode, sc_scores)
                if variant_payload and pipeline._is_better_payload(variant_payload, best_payload):
            ...
            return {
                "decision": decision, "rejection_reason": reason, "quality_score": quality,
                "detected_text": ocr_text, "scene_mode": scene_mode,
                "processed_image": numpy_to_base64(processed_preview),
                "ham_kare": numpy_to_base64(frame),
                "time_ms": (time.perf_counter() - started_at) * 1000,
            }
