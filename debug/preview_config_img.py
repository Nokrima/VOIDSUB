...
class ConfigurableImageProcessor(ImageProcessor):
    ...
    def _process_floating(self, enhanced: np.ndarray, text_mask: np.ndarray) -> np.ndarray:
        ...
        adaptive_gauss = cv2.adaptiveThreshold(enhanced, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 31, gauss_c)
        adaptive_mean = cv2.adaptiveThreshold(enhanced, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 25, mean_c)
    ...
    def process(self, image: np.ndarray, scene_mode: str = "striped") -> tuple[np.ndarray, int]:
        try:
            ...
            if is_large_frame:
                ...
                if width > max_w:
                    scale = max_w / width
                    img_resized = cv2.resize(image, (max_w, max(int(height * scale), 1)), interpolation=cv2.INTER_AREA)
                else:
            ...
            clip_limit = float(self.cfg.get("clahe_clip_floating", 3.5)) if scene_mode == "floating" else float(self.cfg.get("clahe_clip_striped", 2.0))
            ...
            if scene_mode == "striped" and float(np.mean(text_mask > 0)) < 0.003:
                ...
                boosted = cv2.addWeighted(enhanced, 1.35, cv2.GaussianBlur(enhanced, (0, 0), 1.0), -0.35, 0)
                processed = cv2.max(processed, cv2.threshold(boosted, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1])
